...
import { part9Glossary } from "../content/part9/glossary.ts";
import { isPart9Open, part9CardId, part9LessonIdsInCards, unlockedPart9Ids } from "../app/part9-access.ts";

const directory = new URL("../content/part9/", import.meta.url);
...
function metBefore(lesson) {
  return textCourseNeedsMetWords(lesson)
    ? part9Glossary.filter((entry) => entry.lesson < lesson.id)
    : [];
}
...
// The glossary of this course is cumulative against all eight courses before
// it, so a lesson can meet nothing new. Such a lesson is its text and is read,
// not drilled — and that is normal, not a hole in the data. Two here bring
// none: no cards, no questions, and the lesson opens on reading.
...
// Twenty-one lessons of this course bring one or two words, and out of those
...
test("the manifest agrees with the lessons", () => {
  assert.equal(part9Summaries.length, lessons.length);
  for (const lesson of lessons) {
    const summary = part9Summaries.find((item) => item.id === lesson.id);
...
test("the glossary holds the whole course in lesson order", () => {
  ...
  assert.deepEqual(part9Glossary, words);
});

// The ninth course waits for the whole eighth one — the chain every course
// here follows. أُصُول الْفِقْه comes last because it reasons from every course
// before it at once: the wording of a verse, the chain of a report, the قِيَاس
// that carries a rule across.
test("the ninth course waits for the whole eighth one", () => {
  const shelf = [{ id: 1 }, { id: 2 }, { id: 3 }];
  assert.equal(isPart9Open(shelf, {}), false);
  assert.equal(isPart9Open(shelf, { 1: 30, 2: 20 }), false);
  assert.equal(isPart9Open(shelf, { 1: 30, 2: 20, 3: 10 }), true);
  // An empty shelf is not "finished" — the course would then open by itself.
  assert.equal(isPart9Open([], {}), false);
});

test("the ninth course runs in order and never takes ground back", () => {
  const empty = { part9Scores: {}, part9Sessions: {}, cards: {} };
  assert.equal(unlockedPart9Ids(part9Summaries, empty, false).size, 0);

  const opened = unlockedPart9Ids(part9Summaries, empty, true);
  assert.equal(opened.has(1), true, "первый урок должен открыться");
  assert.equal(opened.has(2), false, "второй ждёт первого");

  const oneDone = { ...empty, part9Scores: { 1: 29 } };
  const next = unlockedPart9Ids(part9Summaries, oneDone, true);
  assert.equal(next.has(2), true);
  assert.equal(next.has(3), false);

  const roaming = { ...empty, cards: { [part9CardId(5, 0, "ar-ru")]: {} } };
  const kept = unlockedPart9Ids(part9Summaries, roaming, true);
  for (const id of [1, 2, 3, 4, 5]) assert.equal(kept.has(id), true, `урок ${id} закрылся`);
});

test("a ninth-course card says which lesson it came from", () => {
  assert.equal(part9CardId(7, 12, "ru-ar"), "p9-lesson-7-word-12-ru-ar");
  const cards = {
    "p9-lesson-3-word-0-ar-ru": {},
    "p9-lesson-9-word-4-ru-ar": {},
    // The eight courses before this one share the box and do not belong here.
  ...
  assert.deepEqual(part9LessonIdsInCards(cards).sort((a, b) => a - b), [3, 9]);
});
...
// in at a hundred percent — every word of every lesson — and the bar stays where
// the courses before it hold it: a later book need not be as immaculate.
//
...
// The longest fragment of these books is 73 words: أُصُول states a rule, gives
// its evidence and turns it over, all in a paragraph. Much beyond that is read
// as a page instead.
...
// The ninth course's progress travels between devices and is stored under
// keys of its own; otherwise a merge or a backup would drop it in silence.
test("the ninth course is carried by the store, the merge and the backup", async () => {
  ...
  assert.match(store, /shifahiya-p9-lesson-\$\{id\}/);
  assert.match(store, /shifahiya-p9-session-\$\{id\}/);
  assert.match(store, /finishPart9Lesson/);
  assert.match(merge, /part9Scores: mergeScores/);
  assert.match(merge, /part9Sessions: mergeReadingSessions/);
  assert.match(page, /part9Scores: payload\.part9Scores \?\? \{\}/);
});
