...
import { part7Glossary } from "../content/part7/glossary.ts";
import { isPart7Open, part7CardId, part7LessonIdsInCards, unlockedPart7Ids } from "../app/part7-access.ts";

const directory = new URL("../content/part7/", import.meta.url);
...
function metBefore(lesson) {
  return textCourseNeedsMetWords(lesson)
    ? part7Glossary.filter((entry) => entry.lesson < lesson.id)
    : [];
}
...
// A lesson may bring no new words — one here does, the twenty-third — and it is
// then its text: no cards, no questions, and it opens on the reading step.
...
test("the manifest agrees with the lessons", () => {
  assert.equal(part7Summaries.length, lessons.length);
  for (const lesson of lessons) {
    const summary = part7Summaries.find((item) => item.id === lesson.id);
...
test("the glossary holds the whole course in lesson order", () => {
  ...
  assert.deepEqual(part7Glossary, words);
});

// The seventh course waits for the whole sixth one — the chain every course
// here follows. Logic comes after the language sciences because it weighs an
// argument rather than a sentence.
test("the seventh course waits for the whole sixth one", () => {
  const shelf = [{ id: 1 }, { id: 2 }, { id: 3 }];
  assert.equal(isPart7Open(shelf, {}), false);
  assert.equal(isPart7Open(shelf, { 1: 30, 2: 20 }), false);
  assert.equal(isPart7Open(shelf, { 1: 30, 2: 20, 3: 10 }), true);
  // An empty shelf is not "finished" — the course would then open by itself.
  assert.equal(isPart7Open([], {}), false);
});

test("the seventh course runs in order and never takes ground back", () => {
  const empty = { part7Scores: {}, part7Sessions: {}, cards: {} };
  assert.equal(unlockedPart7Ids(part7Summaries, empty, false).size, 0);

  const opened = unlockedPart7Ids(part7Summaries, empty, true);
  assert.equal(opened.has(1), true, "первый урок должен открыться");
  assert.equal(opened.has(2), false, "второй ждёт первого");

  const oneDone = { ...empty, part7Scores: { 1: 29 } };
  const next = unlockedPart7Ids(part7Summaries, oneDone, true);
  assert.equal(next.has(2), true);
  assert.equal(next.has(3), false);

  const roaming = { ...empty, cards: { [part7CardId(5, 0, "ar-ru")]: {} } };
  const kept = unlockedPart7Ids(part7Summaries, roaming, true);
  for (const id of [1, 2, 3, 4, 5]) assert.equal(kept.has(id), true, `урок ${id} закрылся`);
});

test("a seventh-course card says which lesson it came from", () => {
  assert.equal(part7CardId(7, 12, "ru-ar"), "p7-lesson-7-word-12-ru-ar");
  const cards = {
    "p7-lesson-3-word-0-ar-ru": {},
    "p7-lesson-9-word-4-ru-ar": {},
    // The six courses before this one share the box and do not belong here.
  ...
  assert.deepEqual(part7LessonIdsInCards(cards).sort((a, b) => a - b), [3, 9]);
});
...
// The longest fragment of this book is 39 words: logic states a rule and then
// turns it over. Much beyond a paragraph is read as a page instead.
...
// The seventh course's progress travels between devices and is stored under
// keys of its own; otherwise a merge or a backup would drop it in silence.
test("the seventh course is carried by the store, the merge and the backup", async () => {
  ...
  assert.match(store, /finishPart7Lesson/);
  assert.match(merge, /part7Scores: mergeScores/);
  assert.match(merge, /part7Sessions: mergeReadingSessions/);
  assert.match(page, /part7Scores: payload\.part7Scores \?\? \{\}/);
});
