"use client";

import { useEffect, useMemo, useState } from "react";

type Word = { arabic: string; russian: string; note?: string };
type Question = {
  prompt: string;
  promptLang: "ar" | "ru";
  answer: string;
  options: string[];
  explanation: string;
};
type Lesson = {
  id: number;
  arabicTitle: string;
  title: string;
  description: string;
  tags: string[];
  decks: { title: string; words: Word[] }[];
  questions: Question[];
};

type SavedSession = {
  view: "learn" | "practice";
  lessonId: number;
  deckIndex: number;
  round: number;
  cardIndex: number;
  questionIndex: number;
  score: number;
  mistakes: string[];
};

const lessonOne: Lesson = {
  id: 1,
  arabicTitle: "الدَّرْسُ الأَوَّلُ",
  title: "Он большой. Мы высокие.",
  description: "14 форм · 2 круга повторения · 12 заданий",
  tags: ["Местоимения", "Число", "Первые фразы"],
  decks: [
    {
      title: "Местоимения мужского рода",
      words: [
        { arabic: "هُوَ", russian: "он", note: "мужской род" },
        { arabic: "هُمْ", russian: "они", note: "мужской род" },
        { arabic: "أَنْتَ", russian: "ты", note: "мужской род" },
        { arabic: "أَنْتُمْ", russian: "вы", note: "мужской род" },
        { arabic: "أَنَا", russian: "я" },
        { arabic: "نَحْنُ", russian: "мы" },
      ],
    },
    {
      title: "Признаки: один и несколько",
      words: [
        { arabic: "كَبِيرٌ", russian: "большой" },
        { arabic: "كِبَارٌ", russian: "большие" },
        { arabic: "صَغِيرٌ", russian: "маленький" },
        { arabic: "صِغَارٌ", russian: "маленькие" },
        { arabic: "طَوِيلٌ", russian: "высокий / длинный" },
        { arabic: "طِوَالٌ", russian: "высокие / длинные" },
        { arabic: "قَصِيرٌ", russian: "невысокий / короткий" },
        { arabic: "قِصَارٌ", russian: "невысокие / короткие" },
      ],
    },
  ],
  questions: [
    { prompt: "هُوَ", promptLang: "ar", answer: "он", options: ["он", "они", "мы"], explanation: "هُوَ — «он», местоимение мужского рода." },
    { prompt: "мы", promptLang: "ru", answer: "نَحْنُ", options: ["نَحْنُ", "أَنَا", "أَنْتُمْ"], explanation: "نَحْنُ означает «мы»." },
    { prompt: "كِبَارٌ", promptLang: "ar", answer: "большие", options: ["большие", "маленькие", "большой"], explanation: "كِبَارٌ — форма множественного числа: «большие»." },
    { prompt: "Ты", promptLang: "ru", answer: "أَنْتَ", options: ["أَنْتَ", "أَنَا", "هُوَ"], explanation: "أَنْتَ — «ты» при обращении к мужчине." },
    { prompt: "هُوَ كَبِيرٌ", promptLang: "ar", answer: "Он большой", options: ["Он большой", "Они большие", "Он маленький"], explanation: "В настоящем времени связка «есть» не ставится." },
    { prompt: "Они маленькие", promptLang: "ru", answer: "هُمْ صِغَارٌ", options: ["هُمْ صِغَارٌ", "هُوَ صَغِيرٌ", "نَحْنُ صِغَارٌ"], explanation: "هُمْ — «они», صِغَارٌ — «маленькие»." },
    { prompt: "أَنْتَ طَوِيلٌ", promptLang: "ar", answer: "Ты высокий", options: ["Ты высокий", "Вы высокие", "Я высокий"], explanation: "أَنْتَ — «ты», طَوِيلٌ — «высокий»." },
    { prompt: "Мы невысокие", promptLang: "ru", answer: "نَحْنُ قِصَارٌ", options: ["نَحْنُ قِصَارٌ", "نَحْنُ طِوَالٌ", "هُمْ قِصَارٌ"], explanation: "نَحْنُ — «мы», قِصَارٌ — множественное число." },
    { prompt: "أَنَا صَغِيرٌ", promptLang: "ar", answer: "Я маленький", options: ["Я маленький", "Мы маленькие", "Он маленький"], explanation: "أَنَا означает «я»." },
    { prompt: "Вы большие", promptLang: "ru", answer: "أَنْتُمْ كِبَارٌ", options: ["أَنْتُمْ كِبَارٌ", "هُمْ كِبَارٌ", "أَنْتَ كَبِيرٌ"], explanation: "أَنْتُمْ — «вы», поэтому нужен признак во множественном числе." },
    { prompt: "هُمْ طِوَالٌ", promptLang: "ar", answer: "Они высокие", options: ["Они высокие", "Они невысокие", "Он высокий"], explanation: "هُمْ — «они», طِوَالٌ — «высокие»." },
    { prompt: "هُمْ كَبِيرٌ", promptLang: "ar", answer: "هُمْ كِبَارٌ", options: ["هُمْ كِبَارٌ", "هُوَ كَبِيرٌ", "هُمْ صِغَارٌ"], explanation: "После هُمْ нужна форма множественного числа كِبَارٌ." },
  ],
};

const lessonTwo: Lesson = {
  id: 2,
  arabicTitle: "الدَّرْسُ الثَّانِي",
  title: "Она большая. Они маленькие.",
  description: "14 форм · 2 круга повторения · 12 заданий",
  tags: ["Женский род", "Согласование", "Перевод"],
  decks: [
    {
      title: "Местоимения женского рода",
      words: [
        { arabic: "هِيَ", russian: "она", note: "женский род" },
        { arabic: "هُنَّ", russian: "они", note: "женский род" },
        { arabic: "أَنْتِ", russian: "ты", note: "женский род" },
        { arabic: "أَنْتُنَّ", russian: "вы", note: "женский род" },
        { arabic: "أَنَا", russian: "я" },
        { arabic: "نَحْنُ", russian: "мы" },
      ],
    },
    {
      title: "Женские формы признаков",
      words: [
        { arabic: "كَبِيرَةٌ", russian: "большая" },
        { arabic: "كَبِيرَاتٌ", russian: "большие", note: "женский род" },
        { arabic: "صَغِيرَةٌ", russian: "маленькая" },
        { arabic: "صَغِيرَاتٌ", russian: "маленькие", note: "женский род" },
        { arabic: "طَوِيلَةٌ", russian: "высокая / длинная" },
        { arabic: "طَوِيلَاتٌ", russian: "высокие / длинные", note: "женский род" },
        { arabic: "قَصِيرَةٌ", russian: "невысокая / короткая" },
        { arabic: "قَصِيرَاتٌ", russian: "невысокие / короткие", note: "женский род" },
      ],
    },
  ],
  questions: [
    { prompt: "هِيَ", promptLang: "ar", answer: "она", options: ["она", "они", "ты"], explanation: "هِيَ — «она»." },
    { prompt: "они (женщины)", promptLang: "ru", answer: "هُنَّ", options: ["هُنَّ", "هُمْ", "أَنْتُنَّ"], explanation: "هُنَّ используется, когда речь идёт о женщинах." },
    { prompt: "أَنْتِ", promptLang: "ar", answer: "ты (женщина)", options: ["ты (женщина)", "она", "вы (женщины)"], explanation: "أَنْتِ — обращение к одной женщине." },
    { prompt: "большая", promptLang: "ru", answer: "كَبِيرَةٌ", options: ["كَبِيرَةٌ", "كَبِيرَاتٌ", "صَغِيرَةٌ"], explanation: "Окончание ـة указывает здесь на женский род." },
    { prompt: "هِيَ كَبِيرَةٌ", promptLang: "ar", answer: "Она большая", options: ["Она большая", "Они большие", "Она маленькая"], explanation: "Местоимение и признак согласованы в женском роде." },
    { prompt: "Они большие", promptLang: "ru", answer: "هُنَّ كَبِيرَاتٌ", options: ["هُنَّ كَبِيرَاتٌ", "هِيَ كَبِيرَةٌ", "هُنَّ صَغِيرَاتٌ"], explanation: "Для группы женщин: هُنَّ كَبِيرَاتٌ." },
    { prompt: "أَنْتِ صَغِيرَةٌ", promptLang: "ar", answer: "Ты маленькая", options: ["Ты маленькая", "Вы маленькие", "Она маленькая"], explanation: "أَنْتِ — «ты» при обращении к женщине." },
    { prompt: "Вы высокие", promptLang: "ru", answer: "أَنْتُنَّ طَوِيلَاتٌ", options: ["أَنْتُنَّ طَوِيلَاتٌ", "هُنَّ طَوِيلَاتٌ", "أَنْتِ طَوِيلَةٌ"], explanation: "أَنْتُنَّ — «вы» при обращении к женщинам." },
    { prompt: "هِيَ قَصِيرَةٌ", promptLang: "ar", answer: "Она невысокая", options: ["Она невысокая", "Они невысокие", "Она высокая"], explanation: "قَصِيرَةٌ — женская форма слова «невысокий»." },
    { prompt: "Мы маленькие", promptLang: "ru", answer: "نَحْنُ صَغِيرَاتٌ", options: ["نَحْنُ صَغِيرَاتٌ", "هُنَّ صَغِيرَاتٌ", "نَحْنُ كَبِيرَاتٌ"], explanation: "В контексте женской группы используется صَغِيرَاتٌ." },
    { prompt: "هُنَّ طَوِيلَاتٌ", promptLang: "ar", answer: "Они высокие", options: ["Они высокие", "Они невысокие", "Она высокая"], explanation: "هُنَّ и طَوِيلَاتٌ стоят во множественном числе женского рода." },
    { prompt: "هُنَّ كَبِيرَةٌ", promptLang: "ar", answer: "هُنَّ كَبِيرَاتٌ", options: ["هُنَّ كَبِيرَاتٌ", "هِيَ كَبِيرَةٌ", "هُنَّ صَغِيرَاتٌ"], explanation: "После هُنَّ нужна форма множественного числа كَبِيرَاتٌ." },
  ],
};

const lessonThree: Lesson = {
  id: 3,
  arabicTitle: "الدَّرْسُ الثَّالِثُ",
  title: "Этот мужчина. Те женщины.",
  description: "14 форм · 2 круга повторения · 12 заданий",
  tags: ["Указательные слова", "Люди", "Род и число"],
  decks: [
    {
      title: "Этот, эта, эти и те",
      words: [
        { arabic: "هَذَا", russian: "этот", note: "мужской род" },
        { arabic: "هَذِهِ", russian: "эта", note: "женский род" },
        { arabic: "هَؤُلَاءِ", russian: "эти", note: "для людей" },
        { arabic: "ذَلِكَ", russian: "тот", note: "мужской род" },
        { arabic: "تِلْكَ", russian: "та", note: "женский род" },
        { arabic: "أُولَئِكَ", russian: "те", note: "для людей" },
      ],
    },
    {
      title: "Люди и их признаки",
      words: [
        { arabic: "غَنِيٌّ", russian: "богатый" },
        { arabic: "أَغْنِيَاءُ", russian: "богатые" },
        { arabic: "فَقِيرٌ", russian: "бедный" },
        { arabic: "فُقَرَاءُ", russian: "бедные" },
        { arabic: "رَجُلٌ", russian: "мужчина" },
        { arabic: "رِجَالٌ", russian: "мужчины" },
        { arabic: "اِمْرَأَةٌ", russian: "женщина" },
        { arabic: "نِسَاءٌ", russian: "женщины" },
      ],
    },
  ],
  questions: [
    { prompt: "هَذَا", promptLang: "ar", answer: "этот", options: ["этот", "эта", "тот"], explanation: "هَذَا указывает на один близкий предмет или человека мужского рода." },
    { prompt: "эта", promptLang: "ru", answer: "هَذِهِ", options: ["هَذِهِ", "هَذَا", "تِلْكَ"], explanation: "هَذِهِ — «эта», форма женского рода." },
    { prompt: "أُولَئِكَ", promptLang: "ar", answer: "те", options: ["те", "эти", "тот"], explanation: "أُولَئِكَ — «те» при указании на группу людей вдали." },
    { prompt: "мужчина", promptLang: "ru", answer: "رَجُلٌ", options: ["رَجُلٌ", "رِجَالٌ", "اِمْرَأَةٌ"], explanation: "رَجُلٌ означает «мужчина»." },
    { prompt: "هَذَا رَجُلٌ غَنِيٌّ", promptLang: "ar", answer: "Этот мужчина богатый", options: ["Этот мужчина богатый", "Тот мужчина бедный", "Эти мужчины богатые"], explanation: "هَذَا — «этот», رَجُلٌ — «мужчина», غَنِيٌّ — «богатый»." },
    { prompt: "Эти мужчины бедные", promptLang: "ru", answer: "هَؤُلَاءِ الرِّجَالُ فُقَرَاءُ", options: ["هَؤُلَاءِ الرِّجَالُ فُقَرَاءُ", "أُولَئِكَ الرِّجَالُ أَغْنِيَاءُ", "هَذَا الرَّجُلُ فَقِيرٌ"], explanation: "Для близкой группы людей используется هَؤُلَاءِ." },
    { prompt: "تِلْكَ اِمْرَأَةٌ فَقِيرَةٌ", promptLang: "ar", answer: "Та женщина бедная", options: ["Та женщина бедная", "Эта женщина богатая", "Те женщины бедные"], explanation: "تِلْكَ — «та», форма женского рода." },
    { prompt: "Те женщины богатые", promptLang: "ru", answer: "أُولَئِكَ النِّسَاءُ غَنِيَّاتٌ", options: ["أُولَئِكَ النِّسَاءُ غَنِيَّاتٌ", "هَؤُلَاءِ النِّسَاءُ فَقِيرَاتٌ", "تِلْكَ المَرْأَةُ غَنِيَّةٌ"], explanation: "أُولَئِكَ указывает на удалённую группу людей." },
    { prompt: "هَؤُلَاءِ رِجَالٌ", promptLang: "ar", answer: "Это мужчины", options: ["Это мужчины", "Те мужчины", "Этот мужчина"], explanation: "هَؤُلَاءِ используется для нескольких близких людей." },
    { prompt: "Тот мужчина высокий", promptLang: "ru", answer: "ذَلِكَ الرَّجُلُ طَوِيلٌ", options: ["ذَلِكَ الرَّجُلُ طَوِيلٌ", "هَذَا الرَّجُلُ قَصِيرٌ", "تِلْكَ المَرْأَةُ طَوِيلَةٌ"], explanation: "ذَلِكَ — «тот», الرَّجُلُ — «мужчина»." },
    { prompt: "هَذِهِ المَرْأَةُ قَصِيرَةٌ", promptLang: "ar", answer: "Эта женщина невысокая", options: ["Эта женщина невысокая", "Та женщина высокая", "Эти женщины невысокие"], explanation: "هَذِهِ согласуется с существительным женского рода." },
    { prompt: "هَذَا المَرْأَةُ غَنِيَّةٌ", promptLang: "ar", answer: "هَذِهِ المَرْأَةُ غَنِيَّةٌ", options: ["هَذِهِ المَرْأَةُ غَنِيَّةٌ", "هَذَا الرَّجُلُ غَنِيٌّ", "تِلْكَ المَرْأَةُ فَقِيرَةٌ"], explanation: "С существительным المَرْأَةُ нужно женское указательное слово هَذِهِ." },
  ],
};

const lessonFour: Lesson = {
  id: 4,
  arabicTitle: "الدَّرْسُ الرَّابِعُ",
  title: "Этот юноша — учёный.",
  description: "28 форм · 2 круга повторения · 14 заданий",
  tags: ["Люди", "Возраст", "Занятия и качества"],
  decks: [
    {
      title: "Люди и возраст",
      words: [
        { arabic: "إِنْسَانٌ", russian: "человек" },
        { arabic: "أُنَاسٌ", russian: "люди" },
        { arabic: "شَيْخٌ", russian: "старик / пожилой мужчина" },
        { arabic: "شُيُوخٌ", russian: "старики / пожилые мужчины" },
        { arabic: "عَجُوزٌ", russian: "старуха / пожилая женщина" },
        { arabic: "عَجَائِزُ", russian: "старухи / пожилые женщины" },
        { arabic: "وَلَدٌ", russian: "мальчик / ребёнок" },
        { arabic: "أَوْلَادٌ", russian: "мальчики / дети" },
        { arabic: "صَبِيٌّ", russian: "мальчик" },
        { arabic: "صِبْيَانٌ", russian: "мальчики" },
        { arabic: "صَبِيَّةٌ", russian: "девочка" },
        { arabic: "صَبِيَّاتٌ", russian: "девочки" },
        { arabic: "شَابٌّ", russian: "юноша" },
        { arabic: "شُبَّانٌ", russian: "юноши" },
      ],
    },
    {
      title: "Служение и знания",
      words: [
        { arabic: "شَابَّةٌ", russian: "девушка" },
        { arabic: "شَابَّاتٌ", russian: "девушки" },
        { arabic: "خَادِمٌ", russian: "слуга" },
        { arabic: "خُدَّامٌ", russian: "слуги" },
        { arabic: "خَادِمَةٌ", russian: "служанка" },
        { arabic: "خَادِمَاتٌ", russian: "служанки" },
        { arabic: "عَالِمٌ", russian: "учёный" },
        { arabic: "عُلَمَاءُ", russian: "учёные" },
        { arabic: "عَالِمَةٌ", russian: "учёная" },
        { arabic: "عَالِمَاتٌ", russian: "учёные", note: "женский род" },
        { arabic: "جَاهِلٌ", russian: "невежда / неучёный" },
        { arabic: "جُهَلَاءُ", russian: "невежды" },
        { arabic: "جَاهِلَةٌ", russian: "невежда", note: "женский род" },
        { arabic: "جَاهِلَاتٌ", russian: "невежды", note: "женский род" },
      ],
    },
  ],
  questions: [
    { prompt: "إِنْسَانٌ", promptLang: "ar", answer: "человек", options: ["человек", "люди", "юноша"], explanation: "إِنْسَانٌ означает «человек»." },
    { prompt: "пожилые мужчины", promptLang: "ru", answer: "شُيُوخٌ", options: ["شُيُوخٌ", "شُبَّانٌ", "عَجَائِزُ"], explanation: "شُيُوخٌ — множественное число от شَيْخٌ." },
    { prompt: "عَجَائِزُ", promptLang: "ar", answer: "пожилые женщины", options: ["пожилые женщины", "девушки", "женщины-учёные"], explanation: "عَجَائِزُ — множественное число: «пожилые женщины»." },
    { prompt: "девочки", promptLang: "ru", answer: "صَبِيَّاتٌ", options: ["صَبِيَّاتٌ", "صِبْيَانٌ", "شَابَّاتٌ"], explanation: "صَبِيَّاتٌ — «девочки»." },
    { prompt: "هَذَا الإِنْسَانُ شَيْخٌ", promptLang: "ar", answer: "Этот человек — старик", options: ["Этот человек — старик", "Этот человек — юноша", "Те люди — старики"], explanation: "شَيْخٌ обозначает пожилого мужчину." },
    { prompt: "Те старики — учёные", promptLang: "ru", answer: "أُولَئِكَ الشُّيُوخُ عُلَمَاءُ", options: ["أُولَئِكَ الشُّيُوخُ عُلَمَاءُ", "هَؤُلَاءِ الشُّبَّانُ جُهَلَاءُ", "ذَلِكَ الشَّيْخُ عَالِمٌ"], explanation: "أُولَئِكَ — «те», الشُّيُوخُ — «старики», عُلَمَاءُ — «учёные»." },
    { prompt: "هَؤُلَاءِ الأَوْلَادُ صِبْيَانٌ", promptLang: "ar", answer: "Эти дети — мальчики", options: ["Эти дети — мальчики", "Те дети — девочки", "Этот ребёнок — мальчик"], explanation: "هَؤُلَاءِ используется для близкой группы людей." },
    { prompt: "Эти девушки — учёные", promptLang: "ru", answer: "هَؤُلَاءِ الشَّابَّاتُ عَالِمَاتٌ", options: ["هَؤُلَاءِ الشَّابَّاتُ عَالِمَاتٌ", "أُولَئِكَ الشُّبَّانُ عُلَمَاءُ", "هَذِهِ الشَّابَّةُ جَاهِلَةٌ"], explanation: "Для женской группы используются формы شَابَّاتٌ и عَالِمَاتٌ." },
    { prompt: "ذَلِكَ الخَادِمُ شَابٌّ", promptLang: "ar", answer: "Тот слуга — юноша", options: ["Тот слуга — юноша", "Этот слуга — старик", "Те слуги — юноши"], explanation: "ذَلِكَ — «тот», الخَادِمُ — «слуга», شَابٌّ — «юноша»." },
    { prompt: "Те служанки — пожилые женщины", promptLang: "ru", answer: "أُولَئِكَ الخَادِمَاتُ عَجَائِزُ", options: ["أُولَئِكَ الخَادِمَاتُ عَجَائِزُ", "هَؤُلَاءِ الخُدَّامُ شُيُوخٌ", "تِلْكَ الخَادِمَةُ شَابَّةٌ"], explanation: "الخَادِمَاتُ — «служанки», عَجَائِزُ — «пожилые женщины»." },
    { prompt: "هُمْ شُبَّانٌ", promptLang: "ar", answer: "Они — юноши", options: ["Они — юноши", "Они — старики", "Они — дети"], explanation: "شُبَّانٌ — множественное число от شَابٌّ." },
    { prompt: "Они — невежды", promptLang: "ru", answer: "هُمْ جُهَلَاءُ", options: ["هُمْ جُهَلَاءُ", "هُمْ عُلَمَاءُ", "هُنَّ عَالِمَاتٌ"], explanation: "جُهَلَاءُ — множественное число слова جَاهِلٌ." },
    { prompt: "هُنَّ خَادِمَاتٌ", promptLang: "ar", answer: "Они — служанки", options: ["Они — служанки", "Они — слуги", "Она — служанка"], explanation: "هُنَّ и خَادِمَاتٌ относятся к группе женщин." },
    { prompt: "هَذِهِ الشَّابُ عَالِمٌ", promptLang: "ar", answer: "هَذَا الشَّابُ عَالِمٌ", options: ["هَذَا الشَّابُ عَالِمٌ", "هَذِهِ الشَّابَّةُ عَالِمَةٌ", "هَؤُلَاءِ الشُّبَّانُ عُلَمَاءُ"], explanation: "С существительным الشَّابُ мужского рода используется هَذَا." },
  ],
};

const lessonFive: Lesson = {
  id: 5,
  arabicTitle: "الدَّرْسُ الخَامِسُ",
  title: "Он умный и сильный.",
  description: "20 форм · 2 круга повторения · 14 заданий",
  tags: ["Качества", "Здоровье", "Множественное число"],
  decks: [
    {
      title: "Ум и внешность",
      words: [
        { arabic: "عَاقِلٌ", russian: "умный / разумный" },
        { arabic: "عُقَلَاءُ", russian: "умные / разумные" },
        { arabic: "سَفِيهٌ", russian: "глупый / неразумный" },
        { arabic: "سُفَهَاءُ", russian: "глупые / неразумные" },
        { arabic: "ذَكِيٌّ", russian: "сообразительный" },
        { arabic: "أَذْكِيَاءُ", russian: "сообразительные" },
        { arabic: "غَبِيٌّ", russian: "тупой / несообразительный" },
        { arabic: "أَغْبِيَاءُ", russian: "тупые / несообразительные" },
        { arabic: "سَمِينٌ", russian: "полный" },
        { arabic: "سِمَانٌ", russian: "полные" },
      ],
    },
    {
      title: "Сила и здоровье",
      words: [
        { arabic: "نَحِيفٌ", russian: "худой" },
        { arabic: "نِحَافٌ", russian: "худые" },
        { arabic: "قَوِيٌّ", russian: "сильный" },
        { arabic: "أَقْوِيَاءُ", russian: "сильные" },
        { arabic: "ضَعِيفٌ", russian: "слабый" },
        { arabic: "ضُعَفَاءُ", russian: "слабые" },
        { arabic: "صَحِيحٌ", russian: "здоровый" },
        { arabic: "أَصِحَّاءُ", russian: "здоровые" },
        { arabic: "مَرِيضٌ", russian: "больной" },
        { arabic: "مَرْضَى", russian: "больные" },
      ],
    },
  ],
  questions: [
    { prompt: "عَاقِلٌ", promptLang: "ar", answer: "умный / разумный", options: ["умный / разумный", "глупый", "сильный"], explanation: "عَاقِلٌ означает «умный, разумный»." },
    { prompt: "сообразительные", promptLang: "ru", answer: "أَذْكِيَاءُ", options: ["أَذْكِيَاءُ", "عُقَلَاءُ", "أَغْبِيَاءُ"], explanation: "أَذْكِيَاءُ — множественное число от ذَكِيٌّ." },
    { prompt: "سُفَهَاءُ", promptLang: "ar", answer: "глупые / неразумные", options: ["глупые / неразумные", "умные", "больные"], explanation: "سُفَهَاءُ — множественное число от سَفِيهٌ." },
    { prompt: "худой", promptLang: "ru", answer: "نَحِيفٌ", options: ["نَحِيفٌ", "سَمِينٌ", "ضَعِيفٌ"], explanation: "نَحِيفٌ означает «худой»." },
    { prompt: "هُوَ عَاقِلٌ", promptLang: "ar", answer: "Он умный", options: ["Он умный", "Они умные", "Он глупый"], explanation: "هُوَ — «он», عَاقِلٌ — «умный»." },
    { prompt: "Они сообразительные", promptLang: "ru", answer: "هُمْ أَذْكِيَاءُ", options: ["هُمْ أَذْكِيَاءُ", "هُوَ ذَكِيٌّ", "هُمْ أَغْبِيَاءُ"], explanation: "После هُمْ используется форма множественного числа أَذْكِيَاءُ." },
    { prompt: "هَذَا الرَّجُلُ سَمِينٌ", promptLang: "ar", answer: "Этот мужчина полный", options: ["Этот мужчина полный", "Этот мужчина худой", "Тот мужчина слабый"], explanation: "سَمِينٌ — «полный»." },
    { prompt: "Те мужчины худые", promptLang: "ru", answer: "أُولَئِكَ الرِّجَالُ نِحَافٌ", options: ["أُولَئِكَ الرِّجَالُ نِحَافٌ", "هَؤُلَاءِ الرِّجَالُ سِمَانٌ", "ذَلِكَ الرَّجُلُ نَحِيفٌ"], explanation: "نِحَافٌ — множественное число от نَحِيفٌ." },
    { prompt: "هَؤُلَاءِ الرِّجَالُ أَقْوِيَاءُ", promptLang: "ar", answer: "Эти мужчины сильные", options: ["Эти мужчины сильные", "Те мужчины слабые", "Этот мужчина сильный"], explanation: "أَقْوِيَاءُ означает «сильные»." },
    { prompt: "Эти женщины слабые", promptLang: "ru", answer: "هَؤُلَاءِ النِّسَاءُ ضَعِيفَاتٌ", options: ["هَؤُلَاءِ النِّسَاءُ ضَعِيفَاتٌ", "أُولَئِكَ الرِّجَالُ ضُعَفَاءُ", "هَذِهِ المَرْأَةُ قَوِيَّةٌ"], explanation: "Для группы женщин используется женская форма ضَعِيفَاتٌ." },
    { prompt: "الإِنْسَانُ صَحِيحٌ", promptLang: "ar", answer: "Человек здоров", options: ["Человек здоров", "Человек болен", "Люди здоровы"], explanation: "صَحِيحٌ в этом контексте означает «здоровый»." },
    { prompt: "Больные люди", promptLang: "ru", answer: "النَّاسُ مَرْضَى", options: ["النَّاسُ مَرْضَى", "النَّاسُ أَصِحَّاءُ", "الإِنْسَانُ مَرِيضٌ"], explanation: "مَرْضَى — множественное число слова مَرِيضٌ." },
    { prompt: "هُنَّ صَحِيحَاتٌ", promptLang: "ar", answer: "Они здоровы", options: ["Они здоровы", "Они больны", "Она здорова"], explanation: "صَحِيحَاتٌ — женская форма множественного числа." },
    { prompt: "هُمْ قَوِيٌّ", promptLang: "ar", answer: "هُمْ أَقْوِيَاءُ", options: ["هُمْ أَقْوِيَاءُ", "هُوَ قَوِيٌّ", "هُمْ ضُعَفَاءُ"], explanation: "После هُمْ требуется форма множественного числа أَقْوِيَاءُ." },
  ],
};

const lessonSix: Lesson = {
  id: 6,
  arabicTitle: "الدَّرْسُ السَّادِسُ",
  title: "Мой друг и наши соседи.",
  description: "26 форм · 2 круга повторения · 16 заданий",
  tags: ["Друзья и соседи", "Притяжание", "Окончания"],
  decks: [
    {
      title: "Друг, враг, сосед и гость",
      words: [
        { arabic: "صَدِيقٌ", russian: "друг" },
        { arabic: "أَصْدِقَاءُ", russian: "друзья" },
        { arabic: "عَدُوٌّ", russian: "враг" },
        { arabic: "أَعْدَاءُ", russian: "враги" },
        { arabic: "جَارٌ", russian: "сосед" },
        { arabic: "جِيرَانٌ", russian: "соседи" },
        { arabic: "ضَيْفٌ", russian: "гость" },
        { arabic: "ضُيُوفٌ", russian: "гости" },
        { arabic: "صَدِيقَةٌ", russian: "подруга" },
        { arabic: "صَدِيقَاتٌ", russian: "подруги" },
        { arabic: "عَدُوَّةٌ", russian: "враг", note: "женский род" },
        { arabic: "عَدُوَّاتٌ", russian: "враги", note: "женский род" },
        { arabic: "جَارَةٌ", russian: "соседка" },
        { arabic: "جَارَاتٌ", russian: "соседки" },
        { arabic: "ضَيْفَةٌ", russian: "гостья" },
        { arabic: "ضَيْفَاتٌ", russian: "гостьи" },
      ],
    },
    {
      title: "Чей? Притяжательные окончания",
      words: [
        { arabic: "ـكَ", russian: "твой", note: "обращение к мужчине" },
        { arabic: "ـكِ", russian: "твой", note: "обращение к женщине" },
        { arabic: "ـكُمْ", russian: "ваш", note: "обращение к мужчинам" },
        { arabic: "ـكُنَّ", russian: "ваш", note: "обращение к женщинам" },
        { arabic: "ـهُ", russian: "его" },
        { arabic: "ـهَا", russian: "её" },
        { arabic: "ـهُمْ", russian: "их", note: "мужской род" },
        { arabic: "ـهُنَّ", russian: "их", note: "женский род" },
        { arabic: "ـي", russian: "мой" },
        { arabic: "ـنَا", russian: "наш" },
      ],
    },
  ],
  questions: [
    { prompt: "صَدِيقٌ", promptLang: "ar", answer: "друг", options: ["друг", "враг", "сосед"], explanation: "صَدِيقٌ означает «друг»." },
    { prompt: "соседи", promptLang: "ru", answer: "جِيرَانٌ", options: ["جِيرَانٌ", "أَصْدِقَاءُ", "ضُيُوفٌ"], explanation: "جِيرَانٌ — множественное число от جَارٌ." },
    { prompt: "ضَيْفَاتٌ", promptLang: "ar", answer: "гостьи", options: ["гостьи", "подруги", "соседки"], explanation: "ضَيْفَاتٌ — женская форма множественного числа." },
    { prompt: "враги", promptLang: "ru", answer: "أَعْدَاءُ", options: ["أَعْدَاءُ", "أَصْدِقَاءُ", "عَدُوٌّ"], explanation: "أَعْدَاءُ — множественное число от عَدُوٌّ." },
    { prompt: "صَدِيقِي", promptLang: "ar", answer: "мой друг", options: ["мой друг", "твой друг", "его друг"], explanation: "Окончание ـي передаёт значение «мой»." },
    { prompt: "твой друг (мужчине)", promptLang: "ru", answer: "صَدِيقُكَ", options: ["صَدِيقُكَ", "صَدِيقُكِ", "صَدِيقُهُ"], explanation: "ـكَ используется при обращении к мужчине." },
    { prompt: "صَدِيقَتُهَا", promptLang: "ar", answer: "её подруга", options: ["её подруга", "его подруга", "моя подруга"], explanation: "Окончание ـهَا означает «её»." },
    { prompt: "наши соседи", promptLang: "ru", answer: "جِيرَانُنَا", options: ["جِيرَانُنَا", "جِيرَانُهُمْ", "جَارُكُمْ"], explanation: "Окончание ـنَا передаёт значение «наш, наши»." },
    { prompt: "ضَيْفُهُمْ", promptLang: "ar", answer: "их гость", options: ["их гость", "наш гость", "ваш гость"], explanation: "Окончание ـهُمْ означает «их»." },
    { prompt: "ваши подруги (обращение к женщинам)", promptLang: "ru", answer: "صَدِيقَاتُكُنَّ", options: ["صَدِيقَاتُكُنَّ", "صَدِيقَاتُكُمْ", "صَدِيقَاتُهُنَّ"], explanation: "ـكُنَّ используется при обращении к группе женщин." },
    { prompt: "هَذَا صَدِيقِي", promptLang: "ar", answer: "Это мой друг", options: ["Это мой друг", "Это его друг", "Тот мой сосед"], explanation: "صَدِيقِي состоит из слова «друг» и окончания «мой»." },
    { prompt: "Те мужчины — наши соседи", promptLang: "ru", answer: "أُولَئِكَ الرِّجَالُ جِيرَانُنَا", options: ["أُولَئِكَ الرِّجَالُ جِيرَانُنَا", "هَؤُلَاءِ الرِّجَالُ أَصْدِقَاؤُهُمْ", "ذَلِكَ الرَّجُلُ جَارِي"], explanation: "جِيرَانُنَا означает «наши соседи»." },
    { prompt: "صَدِيقُهُ ذَكِيٌّ", promptLang: "ar", answer: "Его друг сообразительный", options: ["Его друг сообразительный", "Её друг глупый", "Мой друг сильный"], explanation: "ـهُ означает «его», а ذَكِيٌّ — «сообразительный»." },
    { prompt: "Их гостьи здоровы", promptLang: "ru", answer: "ضَيْفَاتُهُنَّ صَحِيحَاتٌ", options: ["ضَيْفَاتُهُنَّ صَحِيحَاتٌ", "ضُيُوفُهُمْ مَرْضَى", "ضَيْفَتُهَا صَحِيحَةٌ"], explanation: "ـهُنَّ означает «их» для группы женщин." },
    { prompt: "أَنْتُمْ أَصْدِقَاؤُنَا", promptLang: "ar", answer: "Вы — наши друзья", options: ["Вы — наши друзья", "Они — ваши друзья", "Мы — их друзья"], explanation: "أَصْدِقَاؤُنَا означает «наши друзья»." },
    { prompt: "هَذِهِ صَدِيقُهَا", promptLang: "ar", answer: "هَذِهِ صَدِيقَتُهَا", options: ["هَذِهِ صَدِيقَتُهَا", "هَذَا صَدِيقُهَا", "هَذِهِ جَارَتُهَا"], explanation: "После هَذِهِ нужно существительное женского рода صَدِيقَةٌ: صَدِيقَتُهَا." },
  ],
};

const lessonSeven: Lesson = {
  id: 7,
  arabicTitle: "الدَّرْسُ السَّابِعُ",
  title: "Моя семья и наши родственники.",
  description: "28 форм · 2 круга повторения · 16 заданий",
  tags: ["Семья", "Родственники", "Имена и притяжание"],
  decks: [
    {
      title: "Члены семьи: один и несколько",
      words: [
        { arabic: "أَبٌ", russian: "отец" },
        { arabic: "آبَاءٌ", russian: "отцы" },
        { arabic: "أُمٌّ", russian: "мать" },
        { arabic: "أُمَّهَاتٌ", russian: "матери" },
        { arabic: "اِبْنٌ", russian: "сын" },
        { arabic: "أَبْنَاءٌ", russian: "сыновья" },
        { arabic: "اِبْنَةٌ", russian: "дочь" },
        { arabic: "بَنَاتٌ", russian: "дочери" },
        { arabic: "أَخٌ", russian: "брат" },
        { arabic: "إِخْوَةٌ", russian: "братья" },
        { arabic: "أُخْتٌ", russian: "сестра" },
        { arabic: "أَخَوَاتٌ", russian: "сёстры" },
        { arabic: "جَدٌّ", russian: "дедушка" },
        { arabic: "أَجْدَادٌ", russian: "дедушки" },
        { arabic: "جَدَّةٌ", russian: "бабушка" },
        { arabic: "جَدَّاتٌ", russian: "бабушки" },
      ],
    },
    {
      title: "Супруги, родство и имена",
      words: [
        { arabic: "زَوْجٌ", russian: "муж" },
        { arabic: "أَزْوَاجٌ", russian: "мужья" },
        { arabic: "زَوْجَةٌ", russian: "жена" },
        { arabic: "زَوْجَاتٌ", russian: "жёны" },
        { arabic: "خَتَنٌ", russian: "зять" },
        { arabic: "أَخْتَانٌ", russian: "зятья" },
        { arabic: "كَنَّةٌ", russian: "невестка" },
        { arabic: "كَنَائِنُ", russian: "невестки" },
        { arabic: "أَبُو عَلِيٍّ", russian: "отец Али", note: "أَبٌ меняется на أَبُو перед именем" },
        { arabic: "أُمُّ وَلِيٍّ", russian: "мать Вали" },
        { arabic: "اِبْنُ مُحَمَّدٍ", russian: "сын Мухаммада" },
        { arabic: "اِبْنَةُ مَحْمُودٍ", russian: "дочь Махмуда" },
      ],
    },
  ],
  questions: [
    { prompt: "أَبٌ", promptLang: "ar", answer: "отец", options: ["отец", "сын", "дедушка"], explanation: "أَبٌ означает «отец»." },
    { prompt: "матери", promptLang: "ru", answer: "أُمَّهَاتٌ", options: ["أُمَّهَاتٌ", "بَنَاتٌ", "جَدَّاتٌ"], explanation: "أُمَّهَاتٌ — множественное число слова أُمٌّ." },
    { prompt: "أَبْنَاءٌ", promptLang: "ar", answer: "сыновья", options: ["сыновья", "братья", "отцы"], explanation: "أَبْنَاءٌ — «сыновья»." },
    { prompt: "сёстры", promptLang: "ru", answer: "أَخَوَاتٌ", options: ["أَخَوَاتٌ", "إِخْوَةٌ", "زَوْجَاتٌ"], explanation: "Арабское слово أَخَوَاتٌ означает «сёстры»." },
    { prompt: "جَدَّةٌ", promptLang: "ar", answer: "бабушка", options: ["бабушка", "мать", "невестка"], explanation: "جَدَّةٌ означает «бабушка»." },
    { prompt: "мужья", promptLang: "ru", answer: "أَزْوَاجٌ", options: ["أَزْوَاجٌ", "أَخْتَانٌ", "أَجْدَادٌ"], explanation: "أَزْوَاجٌ — множественное число слова زَوْجٌ." },
    { prompt: "كَنَّةٌ", promptLang: "ar", answer: "невестка", options: ["невестка", "дочь", "жена"], explanation: "كَنَّةٌ означает «невестка»." },
    { prompt: "зятья", promptLang: "ru", answer: "أَخْتَانٌ", options: ["أَخْتَانٌ", "كَنَائِنُ", "إِخْوَةٌ"], explanation: "أَخْتَانٌ — множественное число слова خَتَنٌ." },
    { prompt: "أَبُو عَلِيٍّ", promptLang: "ar", answer: "отец Али", options: ["отец Али", "сын Али", "дедушка Али"], explanation: "Перед именем форма أَبٌ становится أَبُو: أَبُو عَلِيٍّ." },
    { prompt: "мать Вали", promptLang: "ru", answer: "أُمُّ وَلِيٍّ", options: ["أُمُّ وَلِيٍّ", "اِبْنَةُ وَلِيٍّ", "زَوْجَةُ وَلِيٍّ"], explanation: "أُمُّ وَلِيٍّ буквально означает «мать Вали»." },
    { prompt: "اِبْنُ مُحَمَّدٍ", promptLang: "ar", answer: "сын Мухаммада", options: ["сын Мухаммада", "отец Мухаммада", "брат Мухаммада"], explanation: "اِبْنُ — «сын», مُحَمَّدٍ — имя после него." },
    { prompt: "дочь Махмуда", promptLang: "ru", answer: "اِبْنَةُ مَحْمُودٍ", options: ["اِبْنَةُ مَحْمُودٍ", "أُمُّ مَحْمُودٍ", "أُخْتُ مَحْمُودٍ"], explanation: "اِبْنَةُ مَحْمُودٍ означает «дочь Махмуда»." },
    { prompt: "أَبِي عَالِمٌ", promptLang: "ar", answer: "Мой отец учёный", options: ["Мой отец учёный", "Наш отец богатый", "Его сын умный"], explanation: "Окончание ـي в слове أَبِي передаёт значение «мой»." },
    { prompt: "Наши сыновья здоровы", promptLang: "ru", answer: "أَبْنَاؤُنَا أَصِحَّاءُ", options: ["أَبْنَاؤُنَا أَصِحَّاءُ", "بَنَاتُنَا مَرِيضَاتٌ", "إِخْوَتُهُمْ أَقْوِيَاءُ"], explanation: "أَبْنَاؤُنَا означает «наши сыновья», أَصِحَّاءُ — «здоровы»." },
    { prompt: "أُخْتِي ذَكِيَّةٌ", promptLang: "ar", answer: "Моя сестра сообразительная", options: ["Моя сестра сообразительная", "Моя дочь маленькая", "Его сестра глупая"], explanation: "أُخْتِي — «моя сестра», ذَكِيَّةٌ — «сообразительная»." },
    { prompt: "هَذِهِ أَخِي", promptLang: "ar", answer: "هَذَا أَخِي", options: ["هَذَا أَخِي", "هَذِهِ أُخْتِي", "هَذَا أَبِي"], explanation: "Слово أَخٌ мужского рода, поэтому нужно указательное слово هَذَا: هَذَا أَخِي." },
  ],
};

const lessonEight: Lesson = {
  id: 8,
  arabicTitle: "الدَّرْسُ الثَّامِنُ",
  title: "Наставник, ученик и врач.",
  description: "28 форм · 2 круга повторения · 16 заданий",
  tags: ["Занятия людей", "Качества", "Несогласованное определение"],
  decks: [
    {
      title: "Занятия и люди",
      words: [
        { arabic: "أُسْتَاذٌ", russian: "наставник" },
        { arabic: "أَسَاتِذَةٌ", russian: "наставники" },
        { arabic: "تِلْمِيذٌ", russian: "ученик" },
        { arabic: "تَلَامِيذُ", russian: "ученики" },
        { arabic: "طَبِيبٌ", russian: "врач" },
        { arabic: "أَطِبَّاءُ", russian: "врачи" },
        { arabic: "تَاجِرٌ", russian: "торговец" },
        { arabic: "تُجَّارٌ", russian: "торговцы" },
        { arabic: "صَانِعٌ", russian: "ремесленник" },
        { arabic: "صَنَعَةٌ", russian: "ремесленники", note: "форма множественного числа из книги" },
        { arabic: "مَاهِرٌ", russian: "специалист / умелый" },
        { arabic: "مَهَرَةٌ", russian: "специалисты / умелые" },
      ],
    },
    {
      title: "Нравственные качества и связь слов",
      words: [
        { arabic: "صَالِحٌ", russian: "праведный" },
        { arabic: "صُلَحَاءُ", russian: "праведные" },
        { arabic: "فَاسِقٌ", russian: "порочный" },
        { arabic: "فَسَقَةٌ", russian: "порочные" },
        { arabic: "سَخِيٌّ", russian: "щедрый" },
        { arabic: "أَسْخِيَاءُ", russian: "щедрые" },
        { arabic: "بَخِيلٌ", russian: "скупой" },
        { arabic: "بُخَلَاءُ", russian: "скупые" },
        { arabic: "عَادِلٌ", russian: "справедливый" },
        { arabic: "عُدُولٌ", russian: "справедливые" },
        { arabic: "ظَالِمٌ", russian: "жестокий / несправедливый" },
        { arabic: "ظَلَمَةٌ", russian: "жестокие / несправедливые" },
        { arabic: "أُسْتَاذُ هَذَا التِّلْمِيذِ", russian: "наставник этого ученика", note: "несогласованное определение" },
        { arabic: "أَسَاتِذَةُ هَؤُلَاءِ التَّلَامِيذِ", russian: "наставники этих учеников", note: "несогласованное определение" },
        { arabic: "تِلْمِيذُ ذَلِكَ الأُسْتَاذِ", russian: "ученик того наставника", note: "несогласованное определение" },
        { arabic: "تَلَامِيذُ أُولَئِكَ الأَسَاتِذَةِ", russian: "ученики тех наставников", note: "несогласованное определение" },
      ],
    },
  ],
  questions: [
    { prompt: "أُسْتَاذٌ", promptLang: "ar", answer: "наставник", options: ["наставник", "ученик", "врач"], explanation: "أُسْتَاذٌ означает «наставник»." },
    { prompt: "ученики", promptLang: "ru", answer: "تَلَامِيذُ", options: ["تَلَامِيذُ", "أَسَاتِذَةٌ", "أَطِبَّاءُ"], explanation: "تَلَامِيذُ — множественное число слова تِلْمِيذٌ." },
    { prompt: "أَطِبَّاءُ", promptLang: "ar", answer: "врачи", options: ["врачи", "торговцы", "наставники"], explanation: "أَطِبَّاءُ — «врачи»." },
    { prompt: "торговец", promptLang: "ru", answer: "تَاجِرٌ", options: ["تَاجِرٌ", "صَانِعٌ", "طَبِيبٌ"], explanation: "تَاجِرٌ означает «торговец»." },
    { prompt: "مَهَرَةٌ", promptLang: "ar", answer: "специалисты / умелые", options: ["специалисты / умелые", "праведные", "справедливые"], explanation: "مَهَرَةٌ — множественное число слова مَاهِرٌ." },
    { prompt: "праведный", promptLang: "ru", answer: "صَالِحٌ", options: ["صَالِحٌ", "فَاسِقٌ", "عَادِلٌ"], explanation: "صَالِحٌ означает «праведный»." },
    { prompt: "أَسْخِيَاءُ", promptLang: "ar", answer: "щедрые", options: ["щедрые", "скупые", "жестокие"], explanation: "أَسْخِيَاءُ — множественное число слова سَخِيٌّ." },
    { prompt: "скупой", promptLang: "ru", answer: "بَخِيلٌ", options: ["بَخِيلٌ", "سَخِيٌّ", "ظَالِمٌ"], explanation: "بَخِيلٌ означает «скупой»." },
    { prompt: "عُدُولٌ", promptLang: "ar", answer: "справедливые", options: ["справедливые", "порочные", "праведные"], explanation: "عُدُولٌ — множественное число слова عَادِلٌ." },
    { prompt: "жестокие / несправедливые", promptLang: "ru", answer: "ظَلَمَةٌ", options: ["ظَلَمَةٌ", "فَسَقَةٌ", "صُلَحَاءُ"], explanation: "ظَلَمَةٌ — множественное число слова ظَالِمٌ." },
    { prompt: "أُسْتَاذُ هَذَا التِّلْمِيذِ", promptLang: "ar", answer: "наставник этого ученика", options: ["наставник этого ученика", "ученик этого наставника", "наставники этих учеников"], explanation: "Первое слово обозначает наставника, а следующая группа — «этого ученика»." },
    { prompt: "ученики тех наставников", promptLang: "ru", answer: "تَلَامِيذُ أُولَئِكَ الأَسَاتِذَةِ", options: ["تَلَامِيذُ أُولَئِكَ الأَسَاتِذَةِ", "أَسَاتِذَةُ هَؤُلَاءِ التَّلَامِيذِ", "تِلْمِيذُ ذَلِكَ الأُسْتَاذِ"], explanation: "تَلَامِيذُ — «ученики», أُولَئِكَ الأَسَاتِذَةِ — «тех наставников»." },
    { prompt: "هَذَا الطَّبِيبُ مَاهِرٌ", promptLang: "ar", answer: "Этот врач умелый", options: ["Этот врач умелый", "Тот торговец скупой", "Этот наставник праведный"], explanation: "الطَّبِيبُ — «врач», مَاهِرٌ — «умелый, специалист»." },
    { prompt: "Те торговцы богатые", promptLang: "ru", answer: "أُولَئِكَ التُّجَّارُ أَغْنِيَاءُ", options: ["أُولَئِكَ التُّجَّارُ أَغْنِيَاءُ", "هَؤُلَاءِ الأَطِبَّاءُ فُقَرَاءُ", "ذَلِكَ التَّاجِرُ بَخِيلٌ"], explanation: "التُّجَّارُ — «торговцы», أَغْنِيَاءُ — «богатые»." },
    { prompt: "هَؤُلَاءِ النِّسَاءُ صَالِحَاتٌ", promptLang: "ar", answer: "Эти женщины праведные", options: ["Эти женщины праведные", "Те мужчины порочные", "Эта женщина справедливая"], explanation: "Для группы женщин используется форма صَالِحَاتٌ." },
    { prompt: "هَؤُلَاءِ التِّلْمِيذُ أَذْكِيَاءُ", promptLang: "ar", answer: "هَؤُلَاءِ التَّلَامِيذُ أَذْكِيَاءُ", options: ["هَؤُلَاءِ التَّلَامِيذُ أَذْكِيَاءُ", "هَذَا التِّلْمِيذُ ذَكِيٌّ", "أُولَئِكَ الأَسَاتِذَةُ عُلَمَاءُ"], explanation: "После هَؤُلَاءِ требуется множественное число تَلَامِيذُ: هَؤُلَاءِ التَّلَامِيذُ." },
  ],
};

const lessonNine: Lesson = {
  id: 9,
  arabicTitle: "الدَّرْسُ التَّاسِعُ",
  title: "Старательный учитель.",
  description: "30 форм · 2 круга повторения · 16 заданий",
  tags: ["Учителя", "Качества", "Согласованное определение"],
  decks: [
    {
      title: "Имам, учитель, преподаватель и муэдзин",
      words: [
        { arabic: "إِمَامٌ", russian: "имам" },
        { arabic: "أَئِمَّةٌ", russian: "имамы" },
        { arabic: "مُعَلِّمٌ", russian: "учитель" },
        { arabic: "مُعَلِّمُونَ", russian: "учителя" },
        { arabic: "مُدَرِّسٌ", russian: "преподаватель" },
        { arabic: "مُدَرِّسُونَ", russian: "преподаватели" },
        { arabic: "مُؤَذِّنٌ", russian: "муэдзин" },
        { arabic: "مُؤَذِّنُونَ", russian: "муэдзины" },
        { arabic: "مُتَكَبِّرٌ", russian: "гордый / высокомерный" },
        { arabic: "مُتَكَبِّرُونَ", russian: "гордые / высокомерные" },
        { arabic: "مُتَوَاضِعٌ", russian: "скромный" },
        { arabic: "مُتَوَاضِعُونَ", russian: "скромные" },
      ],
    },
    {
      title: "Состояния, качества и согласование",
      words: [
        { arabic: "مَسْرُورٌ", russian: "радостный" },
        { arabic: "مَسْرُورُونَ", russian: "радостные" },
        { arabic: "مَحْزُونٌ", russian: "печальный" },
        { arabic: "مَحْزُونُونَ", russian: "печальные" },
        { arabic: "مُجْتَهِدٌ", russian: "старательный" },
        { arabic: "مُجْتَهِدُونَ", russian: "старательные" },
        { arabic: "كَسْلَانُ", russian: "ленивый" },
        { arabic: "كُسَالَى", russian: "ленивые" },
        { arabic: "صَادِقٌ", russian: "честный / правдивый" },
        { arabic: "صَادِقُونَ", russian: "честные / правдивые" },
        { arabic: "كَاذِبٌ", russian: "лживый" },
        { arabic: "كَاذِبُونَ", russian: "лживые" },
        { arabic: "الإِمَامُ العَالِمُ", russian: "учёный имам", note: "оба слова имеют определённый артикль" },
        { arabic: "الأَئِمَّةُ العُلَمَاءُ", russian: "учёные имамы", note: "оба слова стоят во множественном числе" },
        { arabic: "المُعَلِّمُ المُجْتَهِدُ", russian: "старательный учитель", note: "согласованное определение" },
        { arabic: "المُعَلِّمُونَ المُجْتَهِدُونَ", russian: "старательные учителя", note: "согласование в числе" },
        { arabic: "المُؤَذِّنُ الكَسْلَانُ", russian: "ленивый муэдзин", note: "согласованное определение" },
        { arabic: "المُؤَذِّنُونَ الكُسَالَى", russian: "ленивые муэдзины", note: "согласование во множественном числе" },
      ],
    },
  ],
  questions: [
    { prompt: "إِمَامٌ", promptLang: "ar", answer: "имам", options: ["имам", "учитель", "муэдзин"], explanation: "إِمَامٌ означает «имам»." },
    { prompt: "преподаватели", promptLang: "ru", answer: "مُدَرِّسُونَ", options: ["مُدَرِّسُونَ", "مُعَلِّمُونَ", "مُؤَذِّنُونَ"], explanation: "مُدَرِّسُونَ — множественное число слова مُدَرِّسٌ." },
    { prompt: "مُتَوَاضِعٌ", promptLang: "ar", answer: "скромный", options: ["скромный", "гордый", "радостный"], explanation: "مُتَوَاضِعٌ означает «скромный»." },
    { prompt: "гордые / высокомерные", promptLang: "ru", answer: "مُتَكَبِّرُونَ", options: ["مُتَكَبِّرُونَ", "مُتَوَاضِعُونَ", "مَسْرُورُونَ"], explanation: "مُتَكَبِّرُونَ — множественное число слова مُتَكَبِّرٌ." },
    { prompt: "مَحْزُونٌ", promptLang: "ar", answer: "печальный", options: ["печальный", "радостный", "ленивый"], explanation: "مَحْزُونٌ означает «печальный»." },
    { prompt: "старательные", promptLang: "ru", answer: "مُجْتَهِدُونَ", options: ["مُجْتَهِدُونَ", "كَاذِبُونَ", "كُسَالَى"], explanation: "مُجْتَهِدُونَ — «старательные»." },
    { prompt: "كُسَالَى", promptLang: "ar", answer: "ленивые", options: ["ленивые", "честные", "печальные"], explanation: "كُسَالَى — множественное число слова كَسْلَانُ." },
    { prompt: "лживый", promptLang: "ru", answer: "كَاذِبٌ", options: ["كَاذِبٌ", "صَادِقٌ", "مُجْتَهِدٌ"], explanation: "كَاذِبٌ означает «лживый»." },
    { prompt: "الإِمَامُ العَالِمُ", promptLang: "ar", answer: "учёный имам", options: ["учёный имам", "старательный учитель", "ленивый муэдзин"], explanation: "العَالِمُ согласуется с الإِمَامُ и означает «учёный»." },
    { prompt: "старательные учителя", promptLang: "ru", answer: "المُعَلِّمُونَ المُجْتَهِدُونَ", options: ["المُعَلِّمُونَ المُجْتَهِدُونَ", "المُؤَذِّنُونَ الكُسَالَى", "الأَئِمَّةُ العُلَمَاءُ"], explanation: "Оба слова стоят во множественном числе и имеют определённый артикль." },
    { prompt: "هَذَا المُدَرِّسُ أُسْتَاذٌ عَالِمٌ", promptLang: "ar", answer: "Этот преподаватель — учёный наставник", options: ["Этот преподаватель — учёный наставник", "Этот учитель — честный мужчина", "Тот имам — ленивый наставник"], explanation: "المُدَرِّسُ — «преподаватель», أُسْتَاذٌ عَالِمٌ — «учёный наставник»." },
    { prompt: "Эти учителя скромные", promptLang: "ru", answer: "هَؤُلَاءِ المُعَلِّمُونَ مُتَوَاضِعُونَ", options: ["هَؤُلَاءِ المُعَلِّمُونَ مُتَوَاضِعُونَ", "أُولَئِكَ المُدَرِّسُونَ مُتَكَبِّرُونَ", "هَذَا المُعَلِّمُ مُجْتَهِدٌ"], explanation: "مُتَوَاضِعُونَ согласуется с группой учителей во множественном числе." },
    { prompt: "ذَلِكَ المُؤَذِّنُ رَجُلٌ غَنِيٌّ", promptLang: "ar", answer: "Тот муэдзин — богатый мужчина", options: ["Тот муэдзин — богатый мужчина", "Этот имам — бедный человек", "Те муэдзины — честные мужчины"], explanation: "ذَلِكَ المُؤَذِّنُ — «тот муэдзин», رَجُلٌ غَنِيٌّ — «богатый мужчина»." },
    { prompt: "Большая женщина печальна", promptLang: "ru", answer: "المَرْأَةُ الكَبِيرَةُ مَحْزُونَةٌ", options: ["المَرْأَةُ الكَبِيرَةُ مَحْزُونَةٌ", "الرَّجُلُ الكَبِيرُ مَسْرُورٌ", "النِّسَاءُ الكَبِيرَاتُ مَحْزُونَاتٌ"], explanation: "Определение الكَبِيرَةُ согласуется со словом المَرْأَةُ в женском роде." },
    { prompt: "هُمْ صَادِقُونَ", promptLang: "ar", answer: "Они честные", options: ["Они честные", "Они лживые", "Он честный"], explanation: "صَادِقُونَ — форма множественного числа: «честные»." },
    { prompt: "المُعَلِّمُ مُجْتَهِدُ", promptLang: "ar", answer: "المُعَلِّمُ المُجْتَهِدُ", options: ["المُعَلِّمُ المُجْتَهِدُ", "مُعَلِّمٌ مُجْتَهِدٌ", "المُعَلِّمُونَ المُجْتَهِدُونَ"], explanation: "В согласованном определении оба слова должны иметь артикль: المُعَلِّمُ المُجْتَهِدُ." },
  ],
};

const lessonTen: Lesson = {
  id: 10,
  arabicTitle: "الدَّرْسُ العَاشِرُ",
  title: "Портной, плотник и кузнец.",
  description: "32 формы · 2 круга повторения · 16 заданий",
  tags: ["Профессии", "Ремёсла", "Притяжательные формы"],
  decks: [
    {
      title: "Ремесленники и профессии",
      words: [
        { arabic: "خَيَّاطٌ", russian: "портной" },
        { arabic: "خَيَّاطُونَ", russian: "портные" },
        { arabic: "نَجَّارٌ", russian: "плотник" },
        { arabic: "نَجَّارُونَ", russian: "плотники" },
        { arabic: "حَدَّادٌ", russian: "кузнец" },
        { arabic: "حَدَّادُونَ", russian: "кузнецы" },
        { arabic: "خَبَّازٌ", russian: "пекарь" },
        { arabic: "خَبَّازُونَ", russian: "пекари" },
        { arabic: "قَصَّابٌ", russian: "мясник" },
        { arabic: "قَصَّابُونَ", russian: "мясники" },
        { arabic: "خَفَّافٌ", russian: "сапожник" },
        { arabic: "خَفَّافُونَ", russian: "сапожники" },
        { arabic: "حَلَّاقٌ", russian: "парикмахер" },
        { arabic: "حَلَّاقُونَ", russian: "парикмахеры" },
      ],
    },
    {
      title: "Другие занятия и притяжание",
      words: [
        { arabic: "فَرَّاشٌ", russian: "уборщик" },
        { arabic: "فَرَّاشُونَ", russian: "уборщики" },
        { arabic: "حَمَّالٌ", russian: "носильщик" },
        { arabic: "حَمَّالُونَ", russian: "носильщики" },
        { arabic: "دَلَّالٌ", russian: "посредник" },
        { arabic: "دَلَّالُونَ", russian: "посредники" },
        { arabic: "فَلَّاحٌ", russian: "земледелец / фермер" },
        { arabic: "فَلَّاحُونَ", russian: "земледельцы / фермеры" },
        { arabic: "مَلَّاحٌ", russian: "моряк" },
        { arabic: "مَلَّاحُونَ", russian: "моряки" },
        { arabic: "بَطَّالٌ", russian: "безработный" },
        { arabic: "بَطَّالُونَ", russian: "безработные" },
        { arabic: "خَيَّاطُهُ", russian: "его портной", note: "окончание ـهُ означает «его»" },
        { arabic: "خَيَّاطُهُمْ", russian: "их портной", note: "окончание ـهُمْ означает «их»" },
        { arabic: "نَجَّارُكَ", russian: "твой плотник", note: "обращение к мужчине" },
        { arabic: "نَجَّارُكُمْ", russian: "ваш плотник", note: "обращение к группе мужчин" },
        { arabic: "حَدَّادِي", russian: "мой кузнец", note: "окончание ـي означает «мой»" },
        { arabic: "حَدَّادُنَا", russian: "наш кузнец", note: "окончание ـنَا означает «наш»" },
      ],
    },
  ],
  questions: [
    { prompt: "خَيَّاطٌ", promptLang: "ar", answer: "портной", options: ["портной", "плотник", "кузнец"], explanation: "خَيَّاطٌ означает «портной»." },
    { prompt: "плотники", promptLang: "ru", answer: "نَجَّارُونَ", options: ["نَجَّارُونَ", "حَدَّادُونَ", "خَبَّازُونَ"], explanation: "نَجَّارُونَ — множественное число слова نَجَّارٌ." },
    { prompt: "قَصَّابٌ", promptLang: "ar", answer: "мясник", options: ["мясник", "пекарь", "сапожник"], explanation: "قَصَّابٌ означает «мясник»." },
    { prompt: "парикмахеры", promptLang: "ru", answer: "حَلَّاقُونَ", options: ["حَلَّاقُونَ", "خَفَّافُونَ", "فَرَّاشُونَ"], explanation: "حَلَّاقُونَ — множественное число слова حَلَّاقٌ." },
    { prompt: "حَمَّالٌ", promptLang: "ar", answer: "носильщик", options: ["носильщик", "уборщик", "посредник"], explanation: "حَمَّالٌ означает «носильщик»." },
    { prompt: "земледельцы / фермеры", promptLang: "ru", answer: "فَلَّاحُونَ", options: ["فَلَّاحُونَ", "مَلَّاحُونَ", "بَطَّالُونَ"], explanation: "فَلَّاحُونَ — множественное число слова فَلَّاحٌ." },
    { prompt: "مَلَّاحٌ", promptLang: "ar", answer: "моряк", options: ["моряк", "фермер", "безработный"], explanation: "مَلَّاحٌ означает «моряк»." },
    { prompt: "безработные", promptLang: "ru", answer: "بَطَّالُونَ", options: ["بَطَّالُونَ", "دَلَّالُونَ", "حَمَّالُونَ"], explanation: "بَطَّالُونَ — множественное число слова بَطَّالٌ." },
    { prompt: "خَيَّاطُهُ", promptLang: "ar", answer: "его портной", options: ["его портной", "их портной", "мой портной"], explanation: "Окончание ـهُ передаёт значение «его»." },
    { prompt: "ваш плотник", promptLang: "ru", answer: "نَجَّارُكُمْ", options: ["نَجَّارُكُمْ", "نَجَّارُكَ", "نَجَّارُنَا"], explanation: "Окончание ـكُمْ означает «ваш» при обращении к группе мужчин." },
    { prompt: "هَذَا الرَّجُلُ خَيَّاطٌ", promptLang: "ar", answer: "Этот мужчина — портной", options: ["Этот мужчина — портной", "Эти мужчины — плотники", "Тот мужчина — кузнец"], explanation: "خَيَّاطٌ — «портной»." },
    { prompt: "Эти женщины — портные", promptLang: "ru", answer: "هَؤُلَاءِ النِّسَاءُ خَيَّاطَاتٌ", options: ["هَؤُلَاءِ النِّسَاءُ خَيَّاطَاتٌ", "هَؤُلَاءِ الرِّجَالُ خَيَّاطُونَ", "هَذِهِ المَرْأَةُ خَيَّاطَةٌ"], explanation: "Для группы женщин используется форма خَيَّاطَاتٌ." },
    { prompt: "جَارِي حَدَّادٌ", promptLang: "ar", answer: "Мой сосед — кузнец", options: ["Мой сосед — кузнец", "Наш сосед — пекарь", "Его друг — плотник"], explanation: "جَارِي — «мой сосед», حَدَّادٌ — «кузнец»." },
    { prompt: "Те сапожники бедные", promptLang: "ru", answer: "أُولَئِكَ الخَفَّافُونَ فُقَرَاءُ", options: ["أُولَئِكَ الخَفَّافُونَ فُقَرَاءُ", "هَؤُلَاءِ الحَمَّالُونَ أَغْنِيَاءُ", "ذَلِكَ الخَفَّافُ مَرِيضٌ"], explanation: "الخَفَّافُونَ — «сапожники», فُقَرَاءُ — «бедные»." },
    { prompt: "أَنْتُمْ خَبَّازُونَ", promptLang: "ar", answer: "Вы — пекари", options: ["Вы — пекари", "Они — мясники", "Мы — кузнецы"], explanation: "خَبَّازُونَ — множественное число: «пекари»." },
    { prompt: "هَؤُلَاءِ الرَّجُلُ نَجَّارُونَ", promptLang: "ar", answer: "هَؤُلَاءِ الرِّجَالُ نَجَّارُونَ", options: ["هَؤُلَاءِ الرِّجَالُ نَجَّارُونَ", "هَذَا الرَّجُلُ نَجَّارٌ", "أُولَئِكَ الرِّجَالُ حَدَّادُونَ"], explanation: "После هَؤُلَاءِ нужно множественное число الرِّجَالُ: هَؤُلَاءِ الرِّجَالُ نَجَّارُونَ." },
  ],
};

const lessonEleven: Lesson = {
  id: 11,
  arabicTitle: "الدَّرْسُ الحَادِي عَشَرَ",
  title: "Он смуглый. Она светлая.",
  description: "36 форм · 2 круга повторения · 72 задания",
  tags: ["Внешность", "Женский род", "Множественное число"],
  decks: [
    {
      title: "Цвет, слух и зрение",
      words: [
        { arabic: "أَزْهَرُ", russian: "светлый" },
        { arabic: "زَهْرَاءُ", russian: "светлая" },
        { arabic: "زُهْرٌ", russian: "светлые" },
        { arabic: "أَسْمَرُ", russian: "смуглый" },
        { arabic: "سَمْرَاءُ", russian: "смуглая" },
        { arabic: "سُمْرٌ", russian: "смуглые" },
        { arabic: "أَشْقَرُ", russian: "рыжий / светловолосый" },
        { arabic: "شَقْرَاءُ", russian: "рыжая / светловолосая" },
        { arabic: "شُقْرٌ", russian: "рыжие / светловолосые" },
        { arabic: "أَخْرَسُ", russian: "немой" },
        { arabic: "خَرْسَاءُ", russian: "немая" },
        { arabic: "خُرْسٌ", russian: "немые" },
        { arabic: "أَصَمُّ", russian: "глухой" },
        { arabic: "صَمَّاءُ", russian: "глухая" },
        { arabic: "صُمٌّ", russian: "глухие" },
        { arabic: "أَعْمَى", russian: "слепой" },
        { arabic: "عَمْيَاءُ", russian: "слепая" },
        { arabic: "عُمْيٌ", russian: "слепые" },
      ],
    },
    {
      title: "Другие физические признаки",
      words: [
        { arabic: "أَحْوَلُ", russian: "косой" },
        { arabic: "حَوْلَاءُ", russian: "косая" },
        { arabic: "حُولٌ", russian: "косые" },
        { arabic: "أَشَلُّ", russian: "парализованный" },
        { arabic: "شَلَّاءُ", russian: "парализованная" },
        { arabic: "شُلٌّ", russian: "парализованные" },
        { arabic: "أَعْرَجُ", russian: "хромой" },
        { arabic: "عَرْجَاءُ", russian: "хромая" },
        { arabic: "عُرْجٌ", russian: "хромые" },
        { arabic: "أَحْدَبُ", russian: "сутулый / горбатый" },
        { arabic: "حَدْبَاءُ", russian: "сутулая / горбатая" },
        { arabic: "حُدْبٌ", russian: "сутулые / горбатые" },
        { arabic: "أَلْثَغُ", russian: "шепелявый" },
        { arabic: "لَثْغَاءُ", russian: "шепелявая" },
        { arabic: "لُثْغٌ", russian: "шепелявые" },
        { arabic: "أَحْمَقُ", russian: "глупый" },
        { arabic: "حَمْقَاءُ", russian: "глупая" },
        { arabic: "حُمْقٌ", russian: "глупые" },
      ],
    },
  ],
  questions: [
    { prompt: "أَزْهَرُ", promptLang: "ar", answer: "светлый", options: ["светлый", "смуглый", "рыжий / светловолосый"], explanation: "أَزْهَرُ означает «светлый»." },
    { prompt: "смуглая", promptLang: "ru", answer: "سَمْرَاءُ", options: ["سَمْرَاءُ", "زَهْرَاءُ", "شَقْرَاءُ"], explanation: "سَمْرَاءُ — женская форма слова أَسْمَرُ." },
    { prompt: "شُقْرٌ", promptLang: "ar", answer: "рыжие / светловолосые", options: ["рыжие / светловолосые", "светлые", "смуглые"], explanation: "شُقْرٌ — множественное число слова أَشْقَرُ." },
    { prompt: "немая", promptLang: "ru", answer: "خَرْسَاءُ", options: ["خَرْسَاءُ", "صَمَّاءُ", "عَمْيَاءُ"], explanation: "خَرْسَاءُ — женская форма слова أَخْرَسُ." },
    { prompt: "صُمٌّ", promptLang: "ar", answer: "глухие", options: ["глухие", "немые", "слепые"], explanation: "صُمٌّ — множественное число слова أَصَمُّ." },
    { prompt: "слепой", promptLang: "ru", answer: "أَعْمَى", options: ["أَعْمَى", "أَصَمُّ", "أَحْوَلُ"], explanation: "أَعْمَى означает «слепой»." },
    { prompt: "حَوْلَاءُ", promptLang: "ar", answer: "косая", options: ["косая", "хромая", "сутулая / горбатая"], explanation: "حَوْلَاءُ — женская форма слова أَحْوَلُ." },
    { prompt: "парализованные", promptLang: "ru", answer: "شُلٌّ", options: ["شُلٌّ", "عُرْجٌ", "حُدْبٌ"], explanation: "شُلٌّ — множественное число слова أَشَلُّ." },
    { prompt: "أَعْرَجُ", promptLang: "ar", answer: "хромой", options: ["хромой", "сутулый / горбатый", "шепелявый"], explanation: "أَعْرَجُ означает «хромой»." },
    { prompt: "шепелявая", promptLang: "ru", answer: "لَثْغَاءُ", options: ["لَثْغَاءُ", "حَمْقَاءُ", "عَرْجَاءُ"], explanation: "لَثْغَاءُ — женская форма слова أَلْثَغُ." },
    { prompt: "هُوَ أَسْمَرُ", promptLang: "ar", answer: "Он смуглый", options: ["Он смуглый", "Она смуглая", "Они смуглые"], explanation: "أَسْمَرُ — форма мужского рода: «смуглый»." },
    { prompt: "Они светлые", promptLang: "ru", answer: "هُمْ زُهْرٌ", options: ["هُمْ زُهْرٌ", "هِيَ زَهْرَاءُ", "هُوَ أَزْهَرُ"], explanation: "Для группы мужчин используется форма множественного числа زُهْرٌ." },
    { prompt: "هَذِهِ المَرْأَةُ صَمَّاءُ", promptLang: "ar", answer: "Эта женщина глухая", options: ["Эта женщина глухая", "Этот мужчина немой", "Эти женщины слепые"], explanation: "صَمَّاءُ — женская форма слова «глухая»." },
    { prompt: "Тот старик парализован", promptLang: "ru", answer: "ذَلِكَ الشَّيْخُ أَشَلُّ", options: ["ذَلِكَ الشَّيْخُ أَشَلُّ", "هَذَا الرَّجُلُ أَعْرَجُ", "تِلْكَ العَجُوزُ حَدْبَاءُ"], explanation: "أَشَلُّ означает «парализованный»." },
    { prompt: "النِّسَاءُ الخُرْسُ صُمٌّ", promptLang: "ar", answer: "Немые женщины глухие", options: ["Немые женщины глухие", "Слепые мужчины немые", "Глухая женщина слепая"], explanation: "الخُرْسُ — «немые», صُمٌّ — «глухие»." },
    { prompt: "هِيَ أَعْمَى", promptLang: "ar", answer: "هِيَ عَمْيَاءُ", options: ["هِيَ عَمْيَاءُ", "هُوَ أَعْمَى", "هُنَّ عُمْيٌ"], explanation: "После هِيَ требуется женская форма عَمْيَاءُ." },
  ],
};

const lessonTwelve: Lesson = {
  id: 12,
  arabicTitle: "الدَّرْسُ الثَّانِي عَشَرَ",
  title: "Он не большой. Они не маленькие.",
  description: "20 форм · 2 круга повторения · 40 заданий",
  tags: ["Отрицание", "Формы لَيْسَ", "Род и число"],
  decks: [
    {
      title: "Личные формы отрицания",
      words: [
        { arabic: "لَيْسَ", russian: "он не / не является" },
        { arabic: "لَيْسُوا", russian: "они не", note: "мужской род" },
        { arabic: "لَيْسَتْ", russian: "она не" },
        { arabic: "لَسْنَ", russian: "они не", note: "женский род" },
        { arabic: "لَسْتَ", russian: "ты не", note: "обращение к мужчине" },
        { arabic: "لَسْتُمْ", russian: "вы не", note: "обращение к мужчинам" },
        { arabic: "لَسْتِ", russian: "ты не", note: "обращение к женщине" },
        { arabic: "لَسْتُنَّ", russian: "вы не", note: "обращение к женщинам" },
        { arabic: "لَسْتُ", russian: "я не" },
        { arabic: "لَسْنَا", russian: "мы не" },
      ],
    },
    {
      title: "Слова и модели отрицательного предложения",
      words: [
        { arabic: "مَا", russian: "не" },
        { arabic: "وَاحِدٌ", russian: "один" },
        { arabic: "أَحَدٌ", russian: "кто-либо / никто", note: "в отрицательном предложении" },
        { arabic: "أَصْلًا", russian: "совсем / нисколько" },
        { arabic: "أَلْبَتَّةَ", russian: "решительно / совершенно" },
        { arabic: "هُوَ لَيْسَ بِكَبِيرٍ", russian: "он не большой", note: "после لَيْسَ используется بِـ" },
        { arabic: "هُمْ لَيْسُوا بِكِبَارٍ", russian: "они не большие", note: "мужской род" },
        { arabic: "هِيَ لَيْسَتْ بِكَبِيرَةٍ", russian: "она не большая" },
        { arabic: "هُنَّ لَسْنَ بِكَبِيرَاتٍ", russian: "они не большие", note: "женский род" },
        { arabic: "لَيْسَ أَحَدٌ بِغَنِيٍّ", russian: "никто не богат" },
      ],
    },
  ],
  questions: [
    { prompt: "لَيْسَ", promptLang: "ar", answer: "он не / не является", options: ["он не / не является", "она не", "они не"], explanation: "لَيْسَ — форма отрицания для мужского рода единственного числа." },
    { prompt: "она не", promptLang: "ru", answer: "لَيْسَتْ", options: ["لَيْسَتْ", "لَيْسَ", "لَسْتِ"], explanation: "لَيْسَتْ используется с подлежащим женского рода." },
    { prompt: "لَيْسُوا", promptLang: "ar", answer: "они не", options: ["они не", "мы не", "вы не"], explanation: "لَيْسُوا используется для группы мужчин." },
    { prompt: "вы не (женщины)", promptLang: "ru", answer: "لَسْتُنَّ", options: ["لَسْتُنَّ", "لَسْتُمْ", "لَسْنَ"], explanation: "لَسْتُنَّ — обращение к группе женщин." },
    { prompt: "لَسْتُ", promptLang: "ar", answer: "я не", options: ["я не", "мы не", "ты не"], explanation: "لَسْتُ означает «я не»." },
    { prompt: "мы не", promptLang: "ru", answer: "لَسْنَا", options: ["لَسْنَا", "لَسْنَ", "لَسْتُ"], explanation: "لَسْنَا означает «мы не»." },
    { prompt: "أَصْلًا", promptLang: "ar", answer: "совсем / нисколько", options: ["совсем / нисколько", "один", "кто-либо / никто"], explanation: "أَصْلًا усиливает отрицание: «совсем, нисколько»." },
    { prompt: "кто-либо / никто", promptLang: "ru", answer: "أَحَدٌ", options: ["أَحَدٌ", "وَاحِدٌ", "مَا"], explanation: "أَحَدٌ в отрицательном предложении передаёт значение «никто»." },
    { prompt: "هُوَ لَيْسَ بِكَبِيرٍ", promptLang: "ar", answer: "он не большой", options: ["он не большой", "она не большая", "они не большие"], explanation: "После لَيْسَ признак присоединён с помощью بِـ: بِكَبِيرٍ." },
    { prompt: "они не большие (женщины)", promptLang: "ru", answer: "هُنَّ لَسْنَ بِكَبِيرَاتٍ", options: ["هُنَّ لَسْنَ بِكَبِيرَاتٍ", "هُمْ لَيْسُوا بِكِبَارٍ", "هِيَ لَيْسَتْ بِكَبِيرَةٍ"], explanation: "Для группы женщин используются هُنَّ и لَسْنَ." },
    { prompt: "أَنْتَ لَسْتَ بِعَالِمٍ", promptLang: "ar", answer: "Ты не учёный", options: ["Ты не учёный", "Вы не учёные", "Он не учёный"], explanation: "لَسْتَ — форма для обращения к одному мужчине." },
    { prompt: "Мы не лживые", promptLang: "ru", answer: "نَحْنُ لَسْنَا بِكَاذِبِينَ", options: ["نَحْنُ لَسْنَا بِكَاذِبِينَ", "هُمْ لَيْسُوا بِصَادِقِينَ", "أَنَا لَسْتُ بِكَاذِبٍ"], explanation: "После لَسْنَا множественная форма с بِـ имеет окончание ـينَ." },
    { prompt: "هَذَا الشَّيْخُ لَيْسَ بِضَعِيفٍ أَصْلًا", promptLang: "ar", answer: "Этот старик совсем не слабый", options: ["Этот старик совсем не слабый", "Этот старик очень слабый", "Тот старик совершенно не сильный"], explanation: "أَصْلًا усиливает отрицание: «совсем не»." },
    { prompt: "Никто не богат", promptLang: "ru", answer: "لَيْسَ أَحَدٌ بِغَنِيٍّ", options: ["لَيْسَ أَحَدٌ بِغَنِيٍّ", "مَا أَحَدٌ غَنِيٌّ", "لَيْسَ وَاحِدٌ بِفَقِيرٍ"], explanation: "لَيْسَ أَحَدٌ بِغَنِيٍّ — модель «никто не богат»." },
    { prompt: "هَؤُلَاءِ الرِّجَالُ لَيْسُوا بِمَرْضَى", promptLang: "ar", answer: "Эти мужчины не больны", options: ["Эти мужчины не больны", "Эти женщины не больны", "Этот мужчина не болен"], explanation: "Для группы мужчин используется لَيْسُوا." },
    { prompt: "هِيَ لَيْسَ بِكَبِيرَةٍ", promptLang: "ar", answer: "هِيَ لَيْسَتْ بِكَبِيرَةٍ", options: ["هِيَ لَيْسَتْ بِكَبِيرَةٍ", "هُوَ لَيْسَ بِكَبِيرٍ", "هُنَّ لَسْنَ بِكَبِيرَاتٍ"], explanation: "С местоимением هِيَ нужна форма женского рода لَيْسَتْ." },
  ],
};

const lessonThirteen: Lesson = {
  id: 13,
  arabicTitle: "الدَّرْسُ الثَّالِثُ عَشَرَ",
  title: "Он терпеливый. Она терпеливая.",
  description: "28 форм · 2 круга повторения · 56 заданий",
  tags: ["Качества", "Одна форма для двух родов", "Каждый и некоторые"],
  decks: [
    {
      title: "Характер: единственное и множественное число",
      words: [
        { arabic: "صَبُورٌ", russian: "терпеливый / терпеливая", note: "одинаково для мужского и женского рода" },
        { arabic: "صُبُرٌ", russian: "терпеливые" },
        { arabic: "عَجُولٌ", russian: "торопливый / торопливая" },
        { arabic: "عُجُلٌ", russian: "торопливые" },
        { arabic: "بَشُوشٌ", russian: "весёлый / весёлая" },
        { arabic: "بُشُشٌ", russian: "весёлые" },
        { arabic: "عَبُوسٌ", russian: "хмурый / хмурая" },
        { arabic: "عُبُسٌ", russian: "хмурые" },
        { arabic: "غَضُوبٌ", russian: "сердитый / сердитая" },
        { arabic: "غُضُبٌ", russian: "сердитые" },
        { arabic: "عَنُودٌ", russian: "упрямый / упрямая" },
        { arabic: "عُنُدٌ", russian: "упрямые" },
        { arabic: "وَقُورٌ", russian: "серьёзный / серьёзная" },
        { arabic: "وُقُرٌ", russian: "серьёзные" },
      ],
    },
    {
      title: "Другие качества и слова количества",
      words: [
        { arabic: "لَعُوبٌ", russian: "игривый / игривая" },
        { arabic: "لُعُبٌ", russian: "игривые" },
        { arabic: "جَسُورٌ", russian: "смелый / смелая" },
        { arabic: "جُسُرٌ", russian: "смелые" },
        { arabic: "حَسُودٌ", russian: "завистливый / завистливая" },
        { arabic: "حُسُدٌ", russian: "завистливые" },
        { arabic: "كُلُّ", russian: "каждый / все" },
        { arabic: "بَعْضُ", russian: "некоторые / часть" },
        { arabic: "أَكْثَرُ", russian: "большинство / больше" },
        { arabic: "جِدًّا", russian: "очень" },
        { arabic: "كُلُّ رَجُلٍ", russian: "каждый мужчина", note: "после كُلُّ слово стоит в родительном падеже" },
        { arabic: "بَعْضُ الرِّجَالِ", russian: "некоторые мужчины" },
        { arabic: "أَكْثَرُ الرِّجَالِ", russian: "большинство мужчин" },
        { arabic: "كُلُّهُمْ", russian: "они все" },
      ],
    },
  ],
  questions: [
    { prompt: "صَبُورٌ", promptLang: "ar", answer: "терпеливый / терпеливая", options: ["терпеливый / терпеливая", "торопливый / торопливая", "хмурый / хмурая"], explanation: "صَبُورٌ употребляется и для мужчины, и для женщины." },
    { prompt: "торопливые", promptLang: "ru", answer: "عُجُلٌ", options: ["عُجُلٌ", "صُبُرٌ", "بُشُشٌ"], explanation: "عُجُلٌ — множественное число слова عَجُولٌ." },
    { prompt: "بَشُوشٌ", promptLang: "ar", answer: "весёлый / весёлая", options: ["весёлый / весёлая", "сердитый / сердитая", "серьёзный / серьёзная"], explanation: "بَشُوشٌ означает «весёлый» или «весёлая»." },
    { prompt: "хмурые", promptLang: "ru", answer: "عُبُسٌ", options: ["عُبُسٌ", "غُضُبٌ", "عُنُدٌ"], explanation: "عُبُسٌ — множественное число слова عَبُوسٌ." },
    { prompt: "عَنُودٌ", promptLang: "ar", answer: "упрямый / упрямая", options: ["упрямый / упрямая", "смелый / смелая", "завистливый / завистливая"], explanation: "عَنُودٌ означает «упрямый» или «упрямая»." },
    { prompt: "серьёзные", promptLang: "ru", answer: "وُقُرٌ", options: ["وُقُرٌ", "جُسُرٌ", "حُسُدٌ"], explanation: "وُقُرٌ — множественное число слова وَقُورٌ." },
    { prompt: "لَعُوبٌ", promptLang: "ar", answer: "игривый / игривая", options: ["игривый / игривая", "весёлый / весёлая", "торопливый / торопливая"], explanation: "لَعُوبٌ означает «игривый» или «игривая»." },
    { prompt: "смелые", promptLang: "ru", answer: "جُسُرٌ", options: ["جُسُرٌ", "لُعُبٌ", "حُسُدٌ"], explanation: "جُسُرٌ — множественное число слова جَسُورٌ." },
    { prompt: "كُلُّ", promptLang: "ar", answer: "каждый / все", options: ["каждый / все", "некоторые / часть", "большинство / больше"], explanation: "كُلُّ означает «каждый» или «все»." },
    { prompt: "некоторые мужчины", promptLang: "ru", answer: "بَعْضُ الرِّجَالِ", options: ["بَعْضُ الرِّجَالِ", "كُلُّ رَجُلٍ", "أَكْثَرُ الرِّجَالِ"], explanation: "بَعْضُ الرِّجَالِ означает «некоторые мужчины»." },
    { prompt: "هِيَ صَبُورٌ", promptLang: "ar", answer: "Она терпеливая", options: ["Она терпеливая", "Он терпеливый", "Они терпеливые"], explanation: "Форма صَبُورٌ одинакова для мужского и женского рода." },
    { prompt: "Они торопливые", promptLang: "ru", answer: "هُمْ عُجُلٌ", options: ["هُمْ عُجُلٌ", "هُوَ عَجُولٌ", "هِيَ عَجُولٌ"], explanation: "Для группы используется форма множественного числа عُجُلٌ." },
    { prompt: "هَذَا المُعَلِّمُ صَبُورٌ جِدًّا", promptLang: "ar", answer: "Этот учитель очень терпеливый", options: ["Этот учитель очень терпеливый", "Этот учитель очень торопливый", "Тот наставник очень серьёзный"], explanation: "جِدًّا усиливает признак и означает «очень»." },
    { prompt: "Большинство богатых старательны", promptLang: "ru", answer: "أَكْثَرُ الأَغْنِيَاءِ مُجْتَهِدُونَ", options: ["أَكْثَرُ الأَغْنِيَاءِ مُجْتَهِدُونَ", "بَعْضُ الفُقَرَاءِ كُسَالَى", "كُلُّ الأَغْنِيَاءِ بُخَلَاءُ"], explanation: "أَكْثَرُ الأَغْنِيَاءِ — «большинство богатых»." },
    { prompt: "كُلُّهُمْ عُلَمَاءُ", promptLang: "ar", answer: "Они все учёные", options: ["Они все учёные", "Некоторые из них учёные", "Большинство из них учителя"], explanation: "كُلُّهُمْ означает «они все»." },
    { prompt: "هِيَ صَبُورَةٌ", promptLang: "ar", answer: "هِيَ صَبُورٌ", options: ["هِيَ صَبُورٌ", "هُوَ صَبُورٌ", "هُنَّ صُبُرٌ"], explanation: "Слово صَبُورٌ одинаково для обоих родов; окончание ـة не добавляется." },
  ],
};

const lessonFourteen: Lesson = {
  id: 14,
  arabicTitle: "الدَّرْسُ الرَّابِعُ عَشَرَ",
  title: "Добрый спутник и счастливый друг.",
  description: "32 формы · 2 круга повторения · 64 задания",
  tags: ["Качества", "Родственники", "Ломаное множественное число"],
  decks: [
    {
      title: "Качества человека",
      words: [
        { arabic: "طَيِّبٌ", russian: "хороший / добрый" },
        { arabic: "طَيِّبُونَ", russian: "хорошие / добрые" },
        { arabic: "خَبِيثٌ", russian: "плохой / грешный" },
        { arabic: "خُبَثَاءُ", russian: "плохие / грешные" },
        { arabic: "سَعِيدٌ", russian: "счастливый" },
        { arabic: "سُعَدَاءُ", russian: "счастливые" },
        { arabic: "تَعِسٌ", russian: "несчастный" },
        { arabic: "تَعِسُونَ", russian: "несчастные" },
        { arabic: "عَزِيزٌ", russian: "уважаемый / дорогой" },
        { arabic: "أَعِزَّةٌ", russian: "уважаемые / дорогие" },
        { arabic: "ذَلِيلٌ", russian: "униженный" },
        { arabic: "أَذِلَّةٌ", russian: "униженные" },
        { arabic: "عَفِيفٌ", russian: "опрятный / целомудренный" },
        { arabic: "أَعِفَّاءُ", russian: "опрятные / целомудренные" },
        { arabic: "نَجِيبٌ", russian: "благородный" },
        { arabic: "نُجَبَاءُ", russian: "благородные" },
      ],
    },
    {
      title: "Спутники, партнёры и родственники",
      words: [
        { arabic: "رَحِيمٌ", russian: "добрый / милосердный" },
        { arabic: "رُحَمَاءُ", russian: "добрые / милосердные" },
        { arabic: "حَلِيمٌ", russian: "кроткий / сдержанный" },
        { arabic: "حُلَمَاءُ", russian: "кроткие / сдержанные" },
        { arabic: "رَفِيقٌ", russian: "спутник" },
        { arabic: "رُفَقَاءُ", russian: "спутники" },
        { arabic: "شَرِيكٌ", russian: "партнёр" },
        { arabic: "شُرَكَاءُ", russian: "партнёры" },
        { arabic: "عَمٌّ", russian: "дядя по отцу" },
        { arabic: "أَعْمَامٌ", russian: "дяди по отцу" },
        { arabic: "عَمَّةٌ", russian: "тётя по отцу" },
        { arabic: "عَمَّاتٌ", russian: "тёти по отцу" },
        { arabic: "خَالٌ", russian: "дядя по матери" },
        { arabic: "أَخْوَالٌ", russian: "дяди по матери" },
        { arabic: "خَالَةٌ", russian: "тётя по матери" },
        { arabic: "خَالَاتٌ", russian: "тёти по матери" },
      ],
    },
  ],
  questions: [
    { prompt: "طَيِّبٌ", promptLang: "ar", answer: "хороший / добрый", options: ["хороший / добрый", "плохой / грешный", "счастливый"], explanation: "طَيِّبٌ означает «хороший, добрый»." },
    { prompt: "плохие / грешные", promptLang: "ru", answer: "خُبَثَاءُ", options: ["خُبَثَاءُ", "طَيِّبُونَ", "تَعِسُونَ"], explanation: "خُبَثَاءُ — множественное число слова خَبِيثٌ." },
    { prompt: "سُعَدَاءُ", promptLang: "ar", answer: "счастливые", options: ["счастливые", "несчастные", "уважаемые / дорогие"], explanation: "سُعَدَاءُ — множественное число слова سَعِيدٌ." },
    { prompt: "униженный", promptLang: "ru", answer: "ذَلِيلٌ", options: ["ذَلِيلٌ", "عَزِيزٌ", "عَفِيفٌ"], explanation: "ذَلِيلٌ означает «униженный»." },
    { prompt: "أَعِفَّاءُ", promptLang: "ar", answer: "опрятные / целомудренные", options: ["опрятные / целомудренные", "благородные", "добрые / милосердные"], explanation: "أَعِفَّاءُ — множественное число слова عَفِيفٌ." },
    { prompt: "благородный", promptLang: "ru", answer: "نَجِيبٌ", options: ["نَجِيبٌ", "حَلِيمٌ", "رَحِيمٌ"], explanation: "نَجِيبٌ означает «благородный»." },
    { prompt: "رُفَقَاءُ", promptLang: "ar", answer: "спутники", options: ["спутники", "партнёры", "дяди по отцу"], explanation: "رُفَقَاءُ — множественное число слова رَفِيقٌ." },
    { prompt: "партнёр", promptLang: "ru", answer: "شَرِيكٌ", options: ["شَرِيكٌ", "رَفِيقٌ", "خَالٌ"], explanation: "شَرِيكٌ означает «партнёр»." },
    { prompt: "عَمَّةٌ", promptLang: "ar", answer: "тётя по отцу", options: ["тётя по отцу", "тётя по матери", "дядя по отцу"], explanation: "عَمَّةٌ — сестра отца." },
    { prompt: "дяди по матери", promptLang: "ru", answer: "أَخْوَالٌ", options: ["أَخْوَالٌ", "أَعْمَامٌ", "خَالَاتٌ"], explanation: "أَخْوَالٌ — множественное число слова خَالٌ." },
    { prompt: "هُوَ طَيِّبٌ", promptLang: "ar", answer: "Он хороший", options: ["Он хороший", "Они хорошие", "Она благородная"], explanation: "طَيِّبٌ — «хороший»." },
    { prompt: "Мой друг счастливый", promptLang: "ru", answer: "صَدِيقِي سَعِيدٌ", options: ["صَدِيقِي سَعِيدٌ", "عَدُوِّي تَعِسٌ", "رَفِيقِي حَلِيمٌ"], explanation: "صَدِيقِي — «мой друг», سَعِيدٌ — «счастливый»." },
    { prompt: "عَمُّ عَلِيٍّ عَفِيفٌ", promptLang: "ar", answer: "Дядя Али по отцу опрятный", options: ["Дядя Али по отцу опрятный", "Дядя Али по матери благородный", "Спутник Али счастливый"], explanation: "عَمٌّ — дядя по отцу." },
    { prompt: "Тёти Вали по матери благородные", promptLang: "ru", answer: "خَالَاتُ وَلِيٍّ نُجَبَاءُ", options: ["خَالَاتُ وَلِيٍّ نُجَبَاءُ", "عَمَّاتُ وَلِيٍّ أَعِفَّاءُ", "أَخْوَالُ وَلِيٍّ رُحَمَاءُ"], explanation: "خَالَاتٌ — тёти по матери, نُجَبَاءُ — благородные." },
    { prompt: "كُلُّ رَجُلٍ عَاقِلٍ رَحِيمٌ", promptLang: "ar", answer: "Каждый разумный мужчина добр", options: ["Каждый разумный мужчина добр", "Некоторые добрые мужчины разумны", "Все мужчины благородны"], explanation: "كُلُّ رَجُلٍ означает «каждый мужчина»." },
    { prompt: "هَذَا الرَّجُلُ خَبِيثُونَ", promptLang: "ar", answer: "هَذَا الرَّجُلُ خَبِيثٌ", options: ["هَذَا الرَّجُلُ خَبِيثٌ", "هَؤُلَاءِ الرِّجَالُ خُبَثَاءُ", "هَذَا الرَّجُلُ طَيِّبٌ"], explanation: "С подлежащим в единственном числе нужна форма خَبِيثٌ." },
  ],
};

const lessonFifteen: Lesson = {
  id: 15,
  arabicTitle: "الدَّرْسُ الخَامِسُ عَشَرَ",
  title: "Известный правитель и занятый министр.",
  description: "24 формы · 2 круга повторения · 48 заданий",
  tags: ["Правители", "Семейное положение", "Союзы"],
  decks: [
    {
      title: "Правители и общественное положение",
      words: [
        { arabic: "مَلِكٌ", russian: "правитель / царь" },
        { arabic: "مُلُوكٌ", russian: "правители / цари" },
        { arabic: "وَزِيرٌ", russian: "министр" },
        { arabic: "وُزَرَاءُ", russian: "министры" },
        { arabic: "أَمِيرٌ", russian: "эмир / правитель" },
        { arabic: "أُمَرَاءُ", russian: "эмиры / правители" },
        { arabic: "سَيِّدٌ", russian: "господин / начальник" },
        { arabic: "سَادَةٌ", russian: "господа / начальники" },
        { arabic: "مَشْغُولٌ", russian: "занятый" },
        { arabic: "مَشْغُولُونَ", russian: "занятые" },
        { arabic: "مَشْهُورٌ", russian: "известный" },
        { arabic: "مَشْهُورُونَ", russian: "известные" },
      ],
    },
    {
      title: "Семейное положение и союзы",
      words: [
        { arabic: "عَزَبٌ", russian: "холостой" },
        { arabic: "أَعْزَابٌ", russian: "холостые" },
        { arabic: "عَزَبَةٌ", russian: "незамужняя" },
        { arabic: "عَزَبَاتٌ", russian: "незамужние" },
        { arabic: "مُتَأَهِّلٌ", russian: "женатый" },
        { arabic: "مُتَأَهِّلُونَ", russian: "женатые" },
        { arabic: "مُتَأَهِّلَةٌ", russian: "замужняя" },
        { arabic: "مُتَأَهِّلَاتٌ", russian: "замужние" },
        { arabic: "وَ", russian: "и / а" },
        { arabic: "بَلْ", russian: "но / напротив" },
        { arabic: "إِمَّا ... أَوْ ...", russian: "либо ... либо ..." },
        { arabic: "أَمَّا ... فَـ ...", russian: "что касается ... то ..." },
      ],
    },
  ],
  questions: [
    { prompt: "مَلِكٌ", promptLang: "ar", answer: "правитель / царь", options: ["правитель / царь", "министр", "эмир / правитель"], explanation: "مَلِكٌ означает «правитель, царь»." },
    { prompt: "министры", promptLang: "ru", answer: "وُزَرَاءُ", options: ["وُزَرَاءُ", "مُلُوكٌ", "أُمَرَاءُ"], explanation: "وُزَرَاءُ — множественное число слова وَزِيرٌ." },
    { prompt: "سَادَةٌ", promptLang: "ar", answer: "господа / начальники", options: ["господа / начальники", "правители / цари", "министры"], explanation: "سَادَةٌ — множественное число слова سَيِّدٌ." },
    { prompt: "известный", promptLang: "ru", answer: "مَشْهُورٌ", options: ["مَشْهُورٌ", "مَشْغُولٌ", "مُتَأَهِّلٌ"], explanation: "مَشْهُورٌ означает «известный»." },
    { prompt: "أَعْزَابٌ", promptLang: "ar", answer: "холостые", options: ["холостые", "женатые", "занятые"], explanation: "أَعْزَابٌ — множественное число слова عَزَبٌ." },
    { prompt: "замужняя", promptLang: "ru", answer: "مُتَأَهِّلَةٌ", options: ["مُتَأَهِّلَةٌ", "عَزَبَةٌ", "مَشْغُولَةٌ"], explanation: "مُتَأَهِّلَةٌ означает «замужняя»." },
    { prompt: "بَلْ", promptLang: "ar", answer: "но / напротив", options: ["но / напротив", "и / а", "либо ... либо ..."], explanation: "بَلْ исправляет или противопоставляет сказанное: «но, напротив»." },
    { prompt: "либо ... либо ...", promptLang: "ru", answer: "إِمَّا ... أَوْ ...", options: ["إِمَّا ... أَوْ ...", "أَمَّا ... فَـ ...", "وَ"], explanation: "إِمَّا ... أَوْ ... означает «либо ... либо ...»." },
    { prompt: "هَذَا المَلِكُ مَشْهُورٌ جِدًّا", promptLang: "ar", answer: "Этот правитель очень известен", options: ["Этот правитель очень известен", "Этот министр очень занят", "Тот эмир очень справедлив"], explanation: "المَلِكُ — «правитель», مَشْهُورٌ — «известный»." },
    { prompt: "Те министры очень заняты", promptLang: "ru", answer: "أُولَئِكَ الوُزَرَاءُ مَشْغُولُونَ جِدًّا", options: ["أُولَئِكَ الوُزَرَاءُ مَشْغُولُونَ جِدًّا", "هَؤُلَاءِ المُلُوكُ مَشْهُورُونَ جِدًّا", "ذَلِكَ الوَزِيرُ مَشْغُولٌ"], explanation: "الوُزَرَاءُ — «министры», مَشْغُولُونَ — «занятые»." },
    { prompt: "هُوَ لَيْسَ بِمُتَأَهِّلٍ بَلْ عَزَبٌ", promptLang: "ar", answer: "Он не женат, а холост", options: ["Он не женат, а холост", "Он не холост, а женат", "Она не замужем"], explanation: "بَلْ вводит исправление: «а, напротив»." },
    { prompt: "Вы не заняты, а безработные", promptLang: "ru", answer: "أَنْتُمْ لَسْتُمْ بِمَشْغُولِينَ بَلْ بَطَّالُونَ", options: ["أَنْتُمْ لَسْتُمْ بِمَشْغُولِينَ بَلْ بَطَّالُونَ", "هُمْ لَيْسُوا بِبَطَّالِينَ بَلْ مَشْغُولُونَ", "أَنَا لَسْتُ بِمَشْغُولٍ"], explanation: "لَسْتُمْ — «вы не», بَلْ — «а, напротив»." },
    { prompt: "الرَّجُلُ إِمَّا شَابٌ أَوْ شَيْخٌ", promptLang: "ar", answer: "Мужчина либо молодой, либо старый", options: ["Мужчина либо молодой, либо старый", "Мужчина не молодой, а старый", "Каждый мужчина молод"], explanation: "إِمَّا ... أَوْ ... задаёт выбор между двумя вариантами." },
    { prompt: "Что касается Али, то он портной", promptLang: "ru", answer: "أَمَّا عَلِيٌّ فَخَيَّاطٌ", options: ["أَمَّا عَلِيٌّ فَخَيَّاطٌ", "عَلِيٌّ خَيَّاطٌ بَلْ نَجَّارٌ", "إِمَّا عَلِيٌّ أَوْ خَالِدٌ"], explanation: "أَمَّا ... فَـ ... означает «что касается ... то ...»." },
    { prompt: "أَبِي سَيِّدٌ وَأَعْمَامِي سَادَةٌ", promptLang: "ar", answer: "Мой отец начальник, а мои дяди начальники", options: ["Мой отец начальник, а мои дяди начальники", "Мой отец министр, а дяди правители", "Мой отец известен, а дяди заняты"], explanation: "وَ соединяет две части предложения и может переводиться как «и» или «а»." },
    { prompt: "هُنَّ مُتَأَهِّلُونَ", promptLang: "ar", answer: "هُنَّ مُتَأَهِّلَاتٌ", options: ["هُنَّ مُتَأَهِّلَاتٌ", "هُمْ مُتَأَهِّلُونَ", "هِيَ مُتَأَهِّلَةٌ"], explanation: "Для группы женщин нужна форма مُتَأَهِّلَاتٌ." },
  ],
};

function buildOptions(answer: string, candidates: string[]) {
  const alternatives = [...new Set(candidates)].filter((item) => item !== answer);
  return [answer, ...alternatives.slice(0, 2)];
}

function expandLessonQuestions(lesson: Lesson): Lesson {
  const words = lesson.decks.flatMap((deck) => deck.words);
  const targetCount = words.length * 2;
  if (lesson.questions.length >= targetCount) return lesson;

  const arabicAnswers = words.map((word) => word.arabic);
  const russianAnswers = words.map((word) => word.russian);
  const generated = words.flatMap<Question>((word) => [
    {
      prompt: word.arabic,
      promptLang: "ar",
      answer: word.russian,
      options: buildOptions(word.russian, russianAnswers),
      explanation: `${word.arabic} означает «${word.russian}».`,
    },
    {
      prompt: word.russian,
      promptLang: "ru",
      answer: word.arabic,
      options: buildOptions(word.arabic, arabicAnswers),
      explanation: `${word.arabic} — правильная арабская форма для «${word.russian}».`,
    },
  ]);

  const needed = targetCount - lesson.questions.length;
  const finalCorrection = lesson.questions.at(-1);
  const contextualQuestions = lesson.questions.slice(0, -1);
  const expandedQuestions = [
    ...contextualQuestions,
    ...generated.slice(0, needed),
    ...(finalCorrection ? [finalCorrection] : []),
  ];

  return {
    ...lesson,
    description: lesson.description.replace(/\d+ заданий/, `${targetCount} заданий`),
    questions: expandedQuestions,
  };
}

const lessons = [
  lessonOne,
  lessonTwo,
  lessonThree,
  lessonFour,
  lessonFive,
  lessonSix,
  lessonSeven,
  lessonEight,
  lessonNine,
  lessonTen,
  lessonEleven,
  lessonTwelve,
  lessonThirteen,
  lessonFourteen,
  lessonFifteen,
].map(expandLessonQuestions);

function speak(text: string) {
  if (typeof window === "undefined" || !("speechSynthesis" in window)) return;
  window.speechSynthesis.cancel();
  const utterance = new SpeechSynthesisUtterance(text);
  utterance.lang = "ar-SA";
  const arabicVoice = window.speechSynthesis
    .getVoices()
    .find((voice) => voice.lang.toLowerCase().startsWith("ar"));
  if (arabicVoice) utterance.voice = arabicVoice;
  utterance.rate = 0.62;
  utterance.pitch = 0.9;
  utterance.volume = 0.88;
  window.speechSynthesis.speak(utterance);
}

function shuffle<T>(items: T[]) {
  const result = [...items];
  for (let index = result.length - 1; index > 0; index -= 1) {
    const target = Math.floor(Math.random() * (index + 1));
    [result[index], result[target]] = [result[target], result[index]];
  }
  return result;
}

export default function Home() {
  const [view, setView] = useState<"home" | "learn" | "practice" | "result">("home");
  const [lessonId, setLessonId] = useState(1);
  const [deckIndex, setDeckIndex] = useState(0);
  const [round, setRound] = useState(1);
  const [cardIndex, setCardIndex] = useState(0);
  const [revealed, setRevealed] = useState(false);
  const [questionIndex, setQuestionIndex] = useState(0);
  const [selected, setSelected] = useState<string | null>(null);
  const [score, setScore] = useState(0);
  const [mistakes, setMistakes] = useState<string[]>([]);
  const [savedScores, setSavedScores] = useState<Record<number, number>>({});
  const [savedSessions, setSavedSessions] = useState<Record<number, SavedSession>>({});
  const [storageReady, setStorageReady] = useState(false);

  const lesson = lessons.find((item) => item.id === lessonId) ?? lessonOne;
  const deck = lesson.decks[deckIndex];
  const words = deck.words;
  const currentWord = words[cardIndex];
  const currentQuestion = lesson.questions[questionIndex];

  function restoreSession(session: SavedSession) {
    setLessonId(session.lessonId);
    setDeckIndex(session.deckIndex);
    setRound(session.round);
    setCardIndex(session.cardIndex);
    setQuestionIndex(session.questionIndex);
    setScore(session.score);
    setMistakes(session.mistakes);
    setSelected(null);
    setRevealed(false);
    setView(session.view);
  }

  useEffect(() => {
    const scores: Record<number, number> = {};
    const sessions: Record<number, SavedSession> = {};
    lessons.forEach((item) => {
      const stored = window.localStorage.getItem(`shifahiya-lesson-${item.id}`);
      if (stored !== null) scores[item.id] = Number(stored);
      const storedLessonSession = window.localStorage.getItem(`shifahiya-session-${item.id}`);
      if (storedLessonSession) {
        try {
          const session = JSON.parse(storedLessonSession) as SavedSession;
          if (session.lessonId === item.id) sessions[item.id] = session;
        } catch {
          window.localStorage.removeItem(`shifahiya-session-${item.id}`);
        }
      }
    });
    setSavedScores(scores);
    setSavedSessions(sessions);
    const storedSession = window.localStorage.getItem("shifahiya-active-session");
    if (storedSession) {
      try {
        const session = JSON.parse(storedSession) as SavedSession;
        if (lessons.some((item) => item.id === session.lessonId)) {
          sessions[session.lessonId] = session;
          window.localStorage.setItem(`shifahiya-session-${session.lessonId}`, JSON.stringify(session));
          setSavedSessions({ ...sessions });
          restoreSession(session);
        }
      } catch {
        window.localStorage.removeItem("shifahiya-active-session");
      }
    }
    setStorageReady(true);
  }, []);

  useEffect(() => {
    if (!storageReady || (view !== "learn" && view !== "practice")) return;
    const session: SavedSession = {
      view,
      lessonId,
      deckIndex,
      round,
      cardIndex,
      questionIndex,
      score,
      mistakes,
    };
    window.localStorage.setItem("shifahiya-active-session", JSON.stringify(session));
    window.localStorage.setItem(`shifahiya-session-${lessonId}`, JSON.stringify(session));
    setSavedSessions((items) => ({ ...items, [lessonId]: session }));
  }, [storageReady, view, lessonId, deckIndex, round, cardIndex, questionIndex, score, mistakes]);

  const progress =
    view === "practice"
      ? ((questionIndex + (selected ? 1 : 0)) / lesson.questions.length) * 100
      : (((deckIndex * 2 + round - 1) * words.length + cardIndex + (revealed ? 1 : 0)) /
          (lesson.decks.length * 2 * words.length)) *
        100;

  const options = useMemo(
    () => shuffle(currentQuestion?.options ?? []),
    [lessonId, questionIndex, currentQuestion],
  );

  function startLesson(id: number) {
    const storedSession = savedSessions[id];
    if (storedSession) {
      restoreSession(storedSession);
      return;
    }
    setLessonId(id);
    setDeckIndex(0);
    setRound(1);
    setCardIndex(0);
    setQuestionIndex(0);
    setScore(0);
    setMistakes([]);
    setSelected(null);
    setRevealed(false);
    setView("learn");
  }

  function nextCard() {
    if (cardIndex < words.length - 1) {
      setCardIndex((value) => value + 1);
    } else if (round === 1) {
      setRound(2);
      setCardIndex(0);
    } else if (deckIndex < lesson.decks.length - 1) {
      setDeckIndex((value) => value + 1);
      setRound(1);
      setCardIndex(0);
    } else {
      setQuestionIndex(0);
      setSelected(null);
      setScore(0);
      setMistakes([]);
      setView("practice");
    }
    setRevealed(false);
  }

  function answer(option: string) {
    if (selected) return;
    setSelected(option);
    if (option === currentQuestion.answer) setScore((value) => value + 1);
    else setMistakes((items) => [...items, currentQuestion.prompt]);
  }

  function nextQuestion() {
    if (questionIndex < lesson.questions.length - 1) {
      setQuestionIndex((value) => value + 1);
      setSelected(null);
      return;
    }
    window.localStorage.setItem(`shifahiya-lesson-${lesson.id}`, String(score));
    window.localStorage.removeItem("shifahiya-active-session");
    window.localStorage.removeItem(`shifahiya-session-${lesson.id}`);
    setSavedScores((items) => ({ ...items, [lesson.id]: score }));
    setSavedSessions((items) => {
      const next = { ...items };
      delete next[lesson.id];
      return next;
    });
    setView("result");
  }

  function resetLesson() {
    window.localStorage.removeItem(`shifahiya-lesson-${lesson.id}`);
    window.localStorage.removeItem("shifahiya-active-session");
    window.localStorage.removeItem(`shifahiya-session-${lesson.id}`);
    setSavedScores((items) => {
      const next = { ...items };
      delete next[lesson.id];
      return next;
    });
    setSavedSessions((items) => {
      const next = { ...items };
      delete next[lesson.id];
      return next;
    });
    setView("home");
  }

  return (
    <main className="app-shell">
      <header className="topbar">
        <button className="brand" onClick={() => setView("home")} aria-label="На главную">
          <span className="brand-mark">ش</span>
          <span><strong>Аш-Шифахия</strong><small>арабский шаг за шагом</small></span>
        </button>
        <div className="streak" title="Серия занятий"><span>✦</span> 1 день</div>
      </header>

      {view !== "home" && view !== "result" && (
        <div className="lesson-progress" aria-label="Прогресс урока">
          <button className="close" onClick={() => setView("home")} aria-label="Закрыть урок">×</button>
          <div className="track"><span style={{ width: `${Math.min(progress, 100)}%` }} /></div>
          <span className="counter">
            {view === "learn" ? `${cardIndex + 1}/${words.length}` : `${questionIndex + 1}/${lesson.questions.length}`}
          </span>
        </div>
      )}

      {view === "home" && (
        <section className="home-view">
          <div className="eyebrow">Ваш путь · 15 из 100 уроков готовы</div>
          <h1>Учимся через<br /><em>повторение и практику</em></h1>
          <p className="lead">Каждая форма встречается дважды в карточках, затем возвращается в переводах и предложениях. Второй урок продолжает первый и вводит женский род.</p>

          <div className="lesson-list">
            {lessons.map((item) => {
              const saved = savedScores[item.id];
              const unfinished = savedSessions[item.id];
              return (
                <div className="lesson-card" key={item.id}>
                  <div className="lesson-number">{String(item.id).padStart(2, "0")}</div>
                  <div className="lesson-copy">
                    <div className="lesson-label">Урок {item.id} · <span dir="rtl">{item.arabicTitle}</span></div>
                    <h2>{item.title}</h2>
                    <p>{item.description}</p>
                    <div className="chips">{item.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
                  </div>
                  <button className="primary" onClick={() => startLesson(item.id)}>
                    {unfinished ? "Продолжить" : saved === undefined ? "Начать урок" : "Повторить"} <span>→</span>
                  </button>
                  {saved !== undefined && <div className="card-score">✓ {saved}/{item.questions.length}</div>}
                </div>
              );
            })}
          </div>

          <div className="principle">
            <span className="quote">“</span>
            <p>Увидели слово. Вспомнили его ещё раз. Затем использовали в предложении.</p>
          </div>
        </section>
      )}

      {view === "learn" && (
        <section className="study-view">
          <div className="stage-label"><span>{deckIndex + 1}</span>{deck.title}</div>
          <div className={`repeat-badge ${round === 2 ? "active" : ""}`}>
            {round === 1 ? "Первое знакомство" : "Повторение без спешки"} · круг {round} из 2
          </div>
          <p className="instruction">{revealed ? "Прочитайте перевод и повторите слово вслух" : round === 1 ? "Познакомьтесь со словом и попробуйте его произнести" : "Попробуйте вспомнить значение до открытия ответа"}</p>
          <article className={`word-card ${revealed ? "is-revealed" : ""}`}>
            <div className="card-ornament">•</div>
            <button className="sound" onClick={() => speak(currentWord.arabic)} aria-label="Прослушать произношение">◖))</button>
            <div className="arabic-word" lang="ar" dir="rtl">{currentWord.arabic}</div>
            <div className="divider" />
            {revealed ? (
              <div className="translation"><strong>{currentWord.russian}</strong>{currentWord.note && <small>{currentWord.note}</small>}</div>
            ) : <div className="hidden-translation">перевод скрыт</div>}
          </article>
          <div className="study-actions">
            {!revealed ? (
              <button className="primary wide" onClick={() => setRevealed(true)}>Показать перевод</button>
            ) : (
              <><button className="secondary" onClick={nextCard}>Пока трудно</button><button className="primary" onClick={nextCard}>Запомнил <span>→</span></button></>
            )}
          </div>
          <button className="text-button" onClick={() => speak(currentWord.arabic)}>Прослушать ещё раз</button>
        </section>
      )}

      {view === "practice" && (
        <section className="practice-view">
          <div className="stage-label"><span>3</span>Закрепляем в заданиях</div>
          <div className="repeat-badge active">Слова возвращаются в обе стороны перевода</div>
          <p className="instruction">{questionIndex === lesson.questions.length - 1 ? "Найдите и исправьте ошибку" : "Выберите правильный ответ"}</p>
          <div className="prompt-card">
            <span>{currentQuestion.promptLang === "ar" ? "Арабский" : "Русский"}</span>
            <strong className={currentQuestion.promptLang === "ar" ? "arabic-prompt" : ""} dir={currentQuestion.promptLang === "ar" ? "rtl" : "ltr"}>{currentQuestion.prompt}</strong>
            {currentQuestion.promptLang === "ar" && <button className="mini-sound" onClick={() => speak(currentQuestion.prompt)} aria-label="Прослушать">◖))</button>}
          </div>
          <div className="options">
            {options.map((option) => {
              const state = selected ? option === currentQuestion.answer ? "correct" : option === selected ? "wrong" : "dimmed" : "";
              return (
                <button key={option} className={state} onClick={() => answer(option)} disabled={!!selected}>
                  <span dir={/[\u0600-\u06FF]/.test(option) ? "rtl" : "ltr"}>{option}</span>
                  {state === "correct" && <b>✓</b>}{state === "wrong" && <b>×</b>}
                </button>
              );
            })}
          </div>
          {selected && (
            <div className={`feedback ${selected === currentQuestion.answer ? "good" : "bad"}`}>
              <div><strong>{selected === currentQuestion.answer ? "Верно!" : "Почти получилось"}</strong><p>{currentQuestion.explanation}</p></div>
              <button className="primary" onClick={nextQuestion}>{questionIndex === lesson.questions.length - 1 ? "Результат" : "Дальше"} <span>→</span></button>
            </div>
          )}
        </section>
      )}

      {view === "result" && (
        <section className="result-view">
          <div className="result-mark">✓</div>
          <div className="eyebrow">Урок {lesson.id} завершён</div>
          <h1>{score >= Math.ceil(lesson.questions.length * 0.75) ? "Материал закреплён!" : "Хороший результат"}</h1>
          <p>{score >= Math.ceil(lesson.questions.length * 0.75) ? "Вы дважды повторили формы и применили их в предложениях. Завтра они вернутся в коротком повторении." : "Ошибочные формы стоит пройти ещё раз — повторение займёт всего несколько минут."}</p>
          <div className="result-grid">
            <div><strong>{score}/{lesson.questions.length}</strong><span>верных ответов</span></div>
            <div><strong>{Math.round((score / lesson.questions.length) * 100)}%</strong><span>точность</span></div>
            <div><strong>{mistakes.length}</strong><span>форм повторить</span></div>
          </div>
          <div className="review-note"><span>◷</span><div><strong>Следующее повторение — завтра</strong><small>Слова, фразы и ваши ошибки · около 3 минут</small></div></div>
          <div className="result-actions">
            <button className="secondary" onClick={resetLesson}>Сбросить результат</button>
            {lesson.id < lessons.length
              ? <button className="primary" onClick={() => startLesson(lesson.id + 1)}>Перейти к уроку {lesson.id + 1} <span>→</span></button>
              : <button className="primary" onClick={() => setView("home")}>Вернуться к курсу <span>→</span></button>}
          </div>
        </section>
      )}

      <footer><span>Учимся осмысленно, повторяем вовремя</span><span lang="ar" dir="rtl">العِلْمُ بِالتَّعَلُّمِ</span></footer>
    </main>
  );
}
