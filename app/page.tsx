...
import { loadPart5Lesson, loadPart5Lessons, loadPart5WordsMetBefore } from "../content/part5/lessons";
import { isPart5Open, part5CardId, part5LessonIdsInCards, unlockedPart5Ids } from "./part5-access";
import { part6Summaries } from "../content/part6/manifest";
import { loadPart6Lesson, loadPart6Lessons, loadPart6WordsMetBefore } from "../content/part6/lessons";
import { isPart6Open, part6CardId, part6LessonIdsInCards, unlockedPart6Ids } from "./part6-access";
 ...
 * comparison, so that the tab it belongs to narrows to a course the screens
 * below actually know.
 */
const isTextCourse = (course: number): course is TextCourseId => course >= 3;

/**
 * All the text courses differ in. A lesson runs identically in every one of
 * them — the new words, one question on each, then the text read whole — so one
 * set of screens serves them all, and what is looked up here is the data, the
 * card addresses and the rows of the store.
 */
const TEXT_COURSES = {
  3: {
    label: "Часть 3",
  ...
  5: {
    label: "Часть 5",
    summaries: part5Summaries,
    loadLesson: loadPart5Lesson,
    cardId: part5CardId,
    wordsMetBefore: loadPart5WordsMetBefore,
    saveSession: progressStore.savePart5Session,
    finishLesson: progressStore.finishPart5Lesson,
  },
  6: {
    label: "Часть 6",
    summaries: part6Summaries,
    loadLesson: loadPart6Lesson,
    cardId: part6CardId,
    wordsMetBefore: loadPart6WordsMetBefore,
    saveSession: progressStore.savePart6Session,
    finishLesson: progressStore.finishPart6Lesson,
  },
  7: {
    label: "Часть 7",
    summaries: part7Summaries,
    loadLesson: loadPart7Lesson,
    cardId: part7CardId,
    wordsMetBefore: loadPart7WordsMetBefore,
  ...
  9: {
    label: "Часть 9",
    summaries: part9Summaries,
    loadLesson: loadPart9Lesson,
    cardId: part9CardId,
    wordsMetBefore: loadPart9WordsMetBefore,
...
/**
 * What each text course waits for. Every one of them opens on the course before
 * it finished whole: there is no paper between them, only the reading that
 * makes the next book readable.
 */
const TEXT_COURSE_GATES: Record<TextCourseId, { title: string; explains: string }> = {
  3: {
    title: "Третья часть открывается по второй",
    explains:
  ...
  4: {
    title: "Четвёртая часть открывается по третьей",
    explains:
  ...
  5: {
    title: "Пятая часть открывается по четвёртой",
    explains:
      "Пройдите все уроки четвёртой части — и грамматика станет доступна. " +
      "Сарф и нахв не учат языку заново, а называют то, что вы уже читаете.",
  },
  6: {
    title: "Шестая часть открывается по пятой",
    explains:
      "Пройдите все уроки пятой части — и балага станет доступна. " +
      "Риторику читают после грамматики: она взвешивает фразу, которую та уже сделала правильной.",
  },
  7: {
    title: "Седьмая часть открывается по шестой",
    explains:
  ...
  9: {
    title: "Девятая часть открывается по восьмой",
    explains:
...
export default function Home() {
  ...
  const part5Scores = stored.part5Scores;
  const part5Sessions = stored.part5Sessions;
  const part6Scores = stored.part6Scores;
  ...
  // And the fifth waits for the fourth: grammar names what has been read, so it
  // comes after the reading rather than before it.
  const part5Ready = isPart5Open(part4Summaries, part4Scores);
  // And the sixth waits for the fifth — here it is also the classical order:
  // rhetoric weighs a sentence that grammar has already made correct.
  const part6Ready = isPart6Open(part5Summaries, part5Scores);
  ...
  const unlockedPart5 = useMemo(
    () => unlockedPart5Ids(part5Summaries, stored, part5Ready),
    [stored, part5Ready],
  );
  ...
  /** Where each text course keeps the learner's place, by course number. */
  const textCourseProgress = useMemo(
    () => ({
      3: { scores: part3Scores, sessions: part3Sessions, unlocked: unlockedPart3, ready: part3Ready },
      4: { scores: part4Scores, sessions: part4Sessions, unlocked: unlockedPart4, ready: part4Ready },
    ...
    [
      ...
      part4Scores, part4Sessions, unlockedPart4, part4Ready,
  ...
  const textCourse = useMemo(
    () => ({ ...TEXT_COURSES[textCourseId], id: textCourseId, ...textCourseProgress[textCourseId] }),
    [textCourseId, textCourseProgress],
  );
  ...
  const wantedPart5Ids = useMemo(() => {
    const resume = Object.values(part5Sessions).sort(
      (a, b) => (b.updatedAt ?? b.lessonId) - (a.updatedAt ?? a.lessonId),
    )[0];
    return [...new Set([...part5LessonIdsInCards(cardProgress), ...(resume ? [resume.lessonId] : [])])]
      .sort((a, b) => a - b)
      .join(",");
  }, [cardProgress, part5Sessions]);

  const wantedPart6Ids = useMemo(() => {
    const resume = Object.values(part6Sessions).sort(
      (a, b) => (b.updatedAt ?? b.lessonId) - (a.updatedAt ?? a.lessonId),
    )[0];
    return [...new Set([...part6LessonIdsInCards(cardProgress), ...(resume ? [resume.lessonId] : [])])]
      .sort((a, b) => a - b)
      .join(",");
  }, [cardProgress, part6Sessions]);

  const wantedPart7Ids = useMemo(() => {
    const resume = Object.values(part7Sessions).sort(
      (a, b) => (b.updatedAt ?? b.lessonId) - (a.updatedAt ?? a.lessonId),
    )[0];
    return [...new Set([...part7LessonIdsInCards(cardProgress), ...(resume ? [resume.lessonId] : [])])]
      .sort((a, b) => a - b)
      .join(",");
  ...
  const wantedPart9Ids = useMemo(() => {
    const resume = Object.values(part9Sessions).sort(
      (a, b) => (b.updatedAt ?? b.lessonId) - (a.updatedAt ?? a.lessonId),
    )[0];
    return [...new Set([...part9LessonIdsInCards(cardProgress), ...(resume ? [resume.lessonId] : [])])]
      .sort((a, b) => a - b)
      .join(",");
  ...
  useEffect(() => {
    if (!wantedPart5Ids) return;

    let cancelled = false;
    loadPart5Lessons(wantedPart5Ids.split(",").map(Number)).then((loaded) => {
      if (cancelled) return;
      setOpenPart5((items) => ({ ...items, ...Object.fromEntries(loaded.map((item) => [item.id, item])) }));
    });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    if (!wantedPart6Ids) return;

    let cancelled = false;
    loadPart6Lessons(wantedPart6Ids.split(",").map(Number)).then((loaded) => {
      if (cancelled) return;
      setOpenPart6((items) => ({ ...items, ...Object.fromEntries(loaded.map((item) => [item.id, item])) }));
    });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    if (!wantedPart7Ids) return;

    let cancelled = false;
    loadPart7Lessons(wantedPart7Ids.split(",").map(Number)).then((loaded) => {
      if (cancelled) return;
      setOpenPart7((items) => ({ ...items, ...Object.fromEntries(loaded.map((item) => [item.id, item])) }));
    });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    if (!wantedPart9Ids) return;

    let cancelled = false;
    loadPart9Lessons(wantedPart9Ids.split(",").map(Number)).then((loaded) => {
      if (cancelled) return;
      setOpenPart9((items) => ({ ...items, ...Object.fromEntries(loaded.map((item) => [item.id, item])) }));
    });
    return () => {
      cancelled = true;
    };
  ...
  async function startTextCourse(
    ...
    const unlocked = textCourseProgress[courseId].unlocked;
    ...
    const sessions = textCourseProgress[courseId].sessions;
  ...
  async function importProgress(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      progressStore.replaceAll({
        ...
        // And the same for the fourth and the fifth.
        part4Scores: payload.part4Scores ?? {},
        part4Sessions: payload.part4Sessions ?? {},
        part5Scores: payload.part5Scores ?? {},
        part5Sessions: payload.part5Sessions ?? {},
        part6Scores: payload.part6Scores ?? {},
