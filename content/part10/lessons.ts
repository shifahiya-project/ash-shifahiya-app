...
// One chunk per lesson, like the courses before it: the tenth course's words
...
export function loadPart10Lesson(id: number): Promise<TextCourseLesson> {
  ...
  if (!loader) return Promise.reject(new Error(`Unknown tenth-course lesson ${id}`));
...
export function loadPart10Lessons(ids: number[]): Promise<TextCourseLesson[]> {
  return Promise.all([...new Set(ids)].map(loadPart10Lesson));
}
 ...
 */
export async function loadPart10WordsMetBefore(lesson: TextCourseLesson): Promise<TextCourseWord[]> {
  if (!textCourseNeedsMetWords(lesson)) return [];
  const { part10Glossary } = await import("./glossary.ts");
  return part10Glossary.filter((entry) => entry.lesson < lesson.id);
}
