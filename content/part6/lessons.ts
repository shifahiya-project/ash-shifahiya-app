...
// One chunk per lesson, like the courses before it: the sixth course's words
...
export function loadPart6Lesson(id: number): Promise<TextCourseLesson> {
  ...
  if (!loader) return Promise.reject(new Error(`Unknown sixth-course lesson ${id}`));
...
export function loadPart6Lessons(ids: number[]): Promise<TextCourseLesson[]> {
  return Promise.all([...new Set(ids)].map(loadPart6Lesson));
}
 ...
 */
export async function loadPart6WordsMetBefore(lesson: TextCourseLesson): Promise<TextCourseWord[]> {
  if (!textCourseNeedsMetWords(lesson)) return [];
  const { part6Glossary } = await import("./glossary.ts");
  return part6Glossary.filter((entry) => entry.lesson < lesson.id);
}
