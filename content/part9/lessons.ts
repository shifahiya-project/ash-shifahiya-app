...
// One chunk per lesson, like the courses before it: the ninth course's words
...
export function loadPart9Lesson(id: number): Promise<TextCourseLesson> {
  ...
  if (!loader) return Promise.reject(new Error(`Unknown ninth-course lesson ${id}`));
...
export function loadPart9Lessons(ids: number[]): Promise<TextCourseLesson[]> {
  return Promise.all([...new Set(ids)].map(loadPart9Lesson));
}
 ...
 */
export async function loadPart9WordsMetBefore(lesson: TextCourseLesson): Promise<TextCourseWord[]> {
  if (!textCourseNeedsMetWords(lesson)) return [];
  const { part9Glossary } = await import("./glossary.ts");
  return part9Glossary.filter((entry) => entry.lesson < lesson.id);
}
