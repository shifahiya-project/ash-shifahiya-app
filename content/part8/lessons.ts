...
// One chunk per lesson, like the courses before it: the eighth course's words
...
export function loadPart8Lesson(id: number): Promise<TextCourseLesson> {
  ...
  if (!loader) return Promise.reject(new Error(`Unknown eighth-course lesson ${id}`));
...
export function loadPart8Lessons(ids: number[]): Promise<TextCourseLesson[]> {
  return Promise.all([...new Set(ids)].map(loadPart8Lesson));
}
 ...
 */
export async function loadPart8WordsMetBefore(lesson: TextCourseLesson): Promise<TextCourseWord[]> {
  if (!textCourseNeedsMetWords(lesson)) return [];
  const { part8Glossary } = await import("./glossary.ts");
  return part8Glossary.filter((entry) => entry.lesson < lesson.id);
}
